...
**Status:** Verified against raw ROM bytes + `tools/extract_func.py` disassembly
           (this session). C lifts in `c/`: `reset_handler.c`, `boot_entry.c`,
           `init_main.c`.
...
[0x0004] = 0xFFFFDFA0                 (initial SP)
   ...
   ▼
Manual_Reset (0x8B8)
   ├─ bsc_init  (0x8CC)   BSC / bus controller init
   ├─ gpio_init (0x8F6)   GPIO port config (0xF720 SFR block)
   └─ resetHandler(0, 0)  @0x4E0
          │
          ▼
resetHandler (0x4E0)                [c/reset_handler.c]
   ├─ resetWatchdog? (0x572)        WDT magic writes
   ├─ hw_init_1 (0x0170)            clock / PLL / FRT
   ├─ hw_init_2 (0x041C)            memory controller / BSC timings
   ├─ hw_init_3 (0x03D4)            additional peripherals
   ├─ cold vs warm detection        magic 0x5AA5A55A @0xFFFFDFFC
   ├─ (cold) checkWatchdogTimer_OVRCOUNT(7) @0x5B0
...
vector_trampoline_set_sp (0x40)     SP=0xFFFFDFA0, jmp @r4
   │
   ├── r4 = 0x6C8   (default)        boot continuation / serial dispatch loop
   ├── r4 = [0x1000] = 0x12B4        app entry -> secondary_boot_init (0x1038)
   └── r4 = [0x7FFF8] = 0xD49C       main entry  -> secondary_boot_main (0xA038)
```

The **app's real entry point is 0xD49C**, reached via `[0x7FFF8]`. The
`0x12B4` → `0x1038` path is the ROM-ID-checked alternate: it validates the
string `"60E1D400"` (pointed to by `[0x7FFFC] = 0x2000`) and then routes
through the same trampoline to 0xD49C. The `0x6C8` default is the
bootloader-style serial dispatch loop (SCI receive → command byte
0xA8/0x88/0x90/0x98/0xA0/0xB0/0xC0 → handler), i.e. the flash/service path.
...
| 0x0004 | `0xFFFFDFA0` | Initial SP |
| 0x0008 | `0x8B8` | Reserved copy |
| 0x000C | `0xFFFFDFA0` | Reserved copy |
| 0x0010–0x0034 | `0x8B4` | General exception / illegal instruction trampoline |
| 0x0038–0x003C | `0xFFFFFFFF` | Unused |

All verified from raw ROM (big-endian longwords).
...
`bsc_init` (0x8CC): writes 15→0xEC20, 0xFFFF→0xEC22/0xEC24, 0x3C04→0xF70A,
0→0xED18, then polls bit 0x8000 of 0xED18 (SDRAM/refresh ready).

`gpio_init` (0x8F6): configures the 0xF720 GPIO block — ports at offsets
0x0/0x2/0x4/0x6 (data/direction) and 0x10/0x12/0x14/0x16 (control), values
0xFFFF/0/EFFF/0x9000/0x3EFF/0x2000 — identical structure to `atu_configure_all_channels`
(0x12BE). Also writes 31→0xF73E, 5→0xF73C.
...
0x4E6  bsr 0x572                ; resetWatchdog? — WDT magic (see below)
...
0x520  (recovered) read [0x7FFFC]: 0xFF sentinel check
         -> [0x7FFFC] != 0xFF && [[0x7FFFC]] != 0xFF  =>  r13 = [0x1000]
...
> **Correction (this session):** earlier draft of `reset_handler.c` read the
> call target from `*(uint16_t*)0x596` (=0x5A1F, a *WDT write magic*), which is
> NOT code. The actual call is the fixed `bsr 0x572`; the literal @0x596 is a
> WDT data word. Fixed in `c/reset_handler.c`.

### checkWatchdogTimer_OVRCOUNT (0x5B0)

Returns non-zero if the watchdog overflowed (used to distinguish a
watchdog-induced reset from a genuine cold start). 62 bytes, verified.

---

## 5. Boot continuation (0x6C8) — serial dispatch loop

Default reset vector when no app entry is found. Sets WDT `[0xDFB8]=0`,
then loops on the SCI receive path:

- setup: jsr 0x364, jsr 0x3E0, jsr 0x3B0 (read a byte pair, compare)
- reads a byte, masks with 0xF8 (`r14`=0xF8), dispatches:
...
| 0x98 | bsr 0x7C0 |
| 0xA0 | jsr 0x7C  (`r8`) |
| 0xB0 | jsr 0x8A  (via [0x7B8]) |
| 0xC0 | jsr 0xC0  (via [0x7BA]) |
| (fallback) | if `[r4]==0xFF && [r5]&0xF8==0xC8` → jsr 0xD8 (via [0x7BC]) |
| else | loop tail: jsr 0x31C, bra 0x6DE |

This is the **flash/service bootloader** protocol loop (command-byte
dispatcher), separate from the app's RTOS. Not further traced this session.
...
Both are the same pattern — set SP then tail-jump:

```
0x40   mov.l [0x48],r15   ; SP = 0xFFFFDFA0
0x42   jmp @r4            ; jump to r4 (never returns)
0x1094 mov.l [0x109C],r15 ; SP = 0xFFFFDFA0
0x1096 jmp @r4
```

### atu_configure_all_channels (0x12BE)

ATU (auto-timer unit) channel configuration on the 0xF720 SFR block:
ports 0xF720/0xF722/0xF724/0xF726 (data/dir) and 0xF730..0xF73E
(control/period) — same layout as `gpio_init` (0x8F6).
...
0xD4A8  jsr 0x4C7A (r4=[0xD9C8]) ; stack_frame_set_sp: mov r4,r15
                            ; SP = [0xD9C8] = 0xFFFF7304
0xD4AC  jsr 0xA038         ; secondary_boot_main — never returns
0xD4B2  bra 0xD4B2         ; infinite loop (idle)
```

### stack_frame_set_sp (0x4C7A)

2 bytes: `rts` with delay slot `mov r4,r15` — sets SP from r4. Sibling
`stack_frame_restore_sp` (0x4C76): `mov r15,r4; rts` — copies SP to r4.
...
(0xA072 onward is a separate warm-restart sibling: sets SR mask, re-runs the
0xF74E bit-set, then a 0x1388×0x239C multiply + port-B mask path. Not part of
the cold boot chain.)
...
0x4C80  sts.l pr,@-r15
0x4C82  bsr 0x5292
0x4C88  [0xED18] = 0xFF            ; SFR write
0x4C8A  bsr 0x4DF6                 ; ubc_breakpoint_config_init
0x4C8E  bsr 0x4E16
0x4C92  bsr 0x4E74
0x4C96  bsr 0x4FDE
        ... (continues through 0x4CBA)
```
...
Verified C lifts: `c/boot_entry.c` (0x3AD8), `c/init_main.c` (0x3E10).
...
`init_main` (0x3E10) then builds the RTOS control block at **0xFFFF72B0**:
...
task_dependency_handler (0x3F10) → task_set_current_ptr (0x3AC0) →
nullsub_2/1 → clear_task_flag_dc/dd (0x3F90/0x3F9C) → [if [0x4B14]≠0]
task_flag_run_A (0x3588) → nullsub_3 → **task_full_context_save (0x3C2A)**,
...
`0xD4B6` (symbol `main??` — actually a warm-restart/validation routine, called
from the 0x64E0 RTOS task):
...
0xD4F0  bra 0xD4F0              ; idle
```

So the ECU can re-enter `resetHandler` in **warm mode** (cold_start=1) after
validation — matching the warm path in `reset_handler.c` (reason → 0xDFA8,
gpio_init, magic store, trampoline exit).
...
- All addresses/literals resolved from raw ROM
  (`roms/stock/60E1D400.bin`, 512 KB, big-endian) with
  `tools/extract_func.py` / `tools/disasm_sh2e.py`.
- `init_main.c` was re-verified against the real disassembly this session:
  prologue `sts.l pr,@-r15; mov r4,r0` and all 8 literal targets
  (0x4B04/0xFFFF72B0/0x4938/0x4990/0x3964/0x3EC0/0x3F10/0x3AC0) match.
- `reset_handler.c` fixed (WDT call target bug, see §4) and exit path
  re-annotated with the verified 0x40 trampoline.
- New: `c/boot_entry.c` (0xD49C entry + 0xA038 second-stage + 0x3AD8 RTOS
  start), compiles clean with host gcc -Wall.

### Remaining open items

1. 0x6C8 bootloader command handlers (0x4C/0x64/0x7C/0x806/0x7C0/0x3E0/0x3B0)
   not individually traced.
2. `peripheral_init_chain_A` (0x4C80) inner calls 0x5292/0x4E16/0x4E74/0x4FDE
   and `secondary_peripheral_initializer` (0xD7B0) internals not yet named.
3. Warm-restart validation chain: what `placeCANRX(0xDBCC)` + `0x636` actually
   validate (DBCC looks like a CAN config table, not a checksum — the earlier
   "checksum" label was wrong; see `validate_data_block_header` @0x636).
