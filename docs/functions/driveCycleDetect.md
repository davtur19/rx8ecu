...
**Out:** Returns in `r0`: 1 if drive cycle detected, 0 if criteria not met ; Uses FP registers (fr14, fr15) for intermediate float computations ; No persistent state modification
...
Save r14, fr14, fr15 to stack; allocate 20 bytes of local frame ; Initialize r14 = 1 (assume success) ; Call `FUN_00066068()` to read sensor validity ; If sensor data invalid (r4 == 0): ; Return 0
(drive cycle not possible without valid sensors) ; Call `returnEngineSpeed()` → store in fr14 (engine RPM as float) ; Call `returnEngineLoad()` → store in fr15 (load % as float) ; Call
`returnCoolantTempGreaterThan71()` → store result (0/1) in stack and r4 ; Compute RPM change: call `subtractAbsolute(fr15, threshold_71.0)` → fr15 = |load% - 71| ; Compute load change: call
`subtractAbsolute(fr14, stack_value)` → fr4 = |rpm_change - something| ; Load threshold 71.0 (float constant at 0x64044 = 0x428E0000) ; Fetch stored threshold values: ; 0x7DB0C ← engine load threshold
(fr2) ; 0x7DB08 ← engine speed threshold (fr1) ; Branch on coolant temp (r4): ; If r4 == 1 (coolant > 71°C): ; Verify: load% ≤ threshold @ 0x7DB0C (if not, r14 = 0) ; Verify: |speed_delta| ≤ threshold
@ 0x7DB08 (if not, r14 = 0) ; If r4 == 0 (coolant ≤ 71°C): ; Verify: load% ≤ threshold @ 0x7DB0C (if not, r14 = 0) ; Verify: |speed_delta| ≤ threshold @ 0x7DB08 (if not, r14 = 0) ; Return r0 = r14 (1
if all criteria met, 0 if any failed)
**Draft C:**
```c
