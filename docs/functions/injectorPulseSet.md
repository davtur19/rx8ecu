...
```
**Status:** med — pulse computation is clear; exact timing/latch semantics and hardware register mapping need verification.
**Uncertainties:** Why pulse is right-shifted by 4 (possibly fixed-point to timer clock conversion) ; Whether 0x8000 threshold has special meaning (overflow? dual-injector mode?) ; Exact hardware register mapping for injector timer channels ; Why two state bytes are cleared; whether they represent completion flags
