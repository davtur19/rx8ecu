...
**Purpose:** Configure the CAN peripheral registers for a specific mailbox. Select between two register banks (channel 0/1).
...
Extract the mailbox number from r4 (mask with 0x0F to get index 0-15) ; Branch based on mailbox < 32: ; Channel 0 (mailbox 0-15): ; Tx/Rx mailbox base: 0xE420 + (mailbox * 20) [offset=(mailbox & 0x0F) <<
...
base: 0xE6B0 + offset ; Push the computed register addresses and input buffer to stack ; Call initializeCANRegisters(reg_addr1, reg_addr2, reg_addr3, buffer, size)
**Draft C:**
```c
...
```
**Status:** med – mailbox selector and offset calculation are clear. The addresses are within the CAN peripheral area (0xE4xx, 0xE6xx expected for the SH7055 HCAN). The exact register semantics require the Denso HCAN spec.
