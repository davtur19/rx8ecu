...
**Purpose:** Control the alternator warning light (dash indicator) based on multiple electrical and charging fault conditions.
...
**Calls:** None
Read the charging fault flag from 0xB663 ; If not set: skip to step 5 (turn light on) ; Read the over-voltage flag from 0xB5FC ; If set (value == 1): turn light on; return ; Read the under-voltage flag from
0xB5FD ; If set (value == 1): turn light on; return ; Read the alternator control fault from 0xB5FE ; If set (value == 1): turn light on; return ; Read the charging/thermal fault from 0xC5DB ; If set (value
