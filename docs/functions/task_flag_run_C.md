...
The task scheduler calls it to mark that the RTOS (or a specific
...
and `task_flag_run_C` (0x35EE) form a set — each sets or clears a different
bit in the same 32-bit register. The set bit shows the active scheduler
entry point.
...
- [x] Emulator test: `test_task_flag_run_C.py` — set/clear bit 15 with a nullsub call, all pass
