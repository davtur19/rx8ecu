...
**Purpose:** Control the immobilizer warning lamp (MIL-style indicator). Manipulate a hardware register with bit-set and bit-clear operations. Manage the CPU interrupt state during register access.
**Inputs:** `r4`: lamp state (0x01 = on, 0x00 = off)
**Out:** Modifies hardware register at 0xF754 (likely an I/O port or control register): ; If r4 == 1: sets bits 64 (0x40) and 32 (0x20) ; If r4 == 0: clears bits 64 and 32 ; Refreshes the register state with `loadStatusRegister` (0x25E80) ; Saves and restores SR (interrupt mask) with helper calls
...
Save r10-r14 to stack ; Load the helper function addresses: ; r10 ← setSR_PARAM (0x2054) ; r11 ← loadStatusRegister_ADDR (0x2064) ; r14 ← setRegister_REG_BIT_VAL (0x4BBC) ; Load the register constants: ; r12
← register address 0xF754 (masked as 0x00E0) ; r13 ← register address 0xF754 (full) ; Test r4 (lamp state): ; If r4 == 1 (lamp ON): ; Call setSR_PARAM to save the interrupt state (stack frame at r15+20) ;
...
```
**Status:** high — register 0xF754 and bit patterns (0x40/0x20) are consistent; the name matches the immobilizer lamp pattern; SR save/restore + register sync = interrupt-protected HW access. Uncertain: bit meanings (64/32 may be count artifacts); ON = one bit or both.
