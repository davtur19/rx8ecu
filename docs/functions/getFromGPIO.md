...
**Calls:** `getSR` (0x00003920) – save SR, disable interrupts (arg r4=0x00B0) ; `setRegister_REG_BIT_VAL` (0x00004BBC) – write port control bits via r4=port_reg, r5=value, r6=?
Save SR, disable interrupts ; Store GPIO pin argument in local stack ; Configure port directions and multiplexing (via three setRegister calls): ; Clear bits in port F002h DDR (and #11 mask) ; Write
...
r4 == 0: call setRegister twice with r5=0x4000 and r5=0x8000 (clear both bits?) ; If r4 == 1: call setRegister with r5=0x4000 only, set bit to 1 ; Read GPIO input state via port register ; Restore SR,
