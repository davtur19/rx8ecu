...
**Inputs:** System state (no explicit parameters; reads from various RAM locations via called functions)
**Out:** EVAP solenoid/canister control writes (via called functions) ; Purge valve control ; Fuel vapor pressure monitoring ; Fault logging (if applicable)
