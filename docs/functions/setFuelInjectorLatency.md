...
**Purpose:** Set the fuel injector pulse latency (dead-time offset) for one of three rotor phases. The rotor is indexed by r4. Write the same value to two paired registers (primary and shadow).
...
Load the structure base address 0xFFFFA094 into r6 ; Test r4 against 0; if true, write r5 to [r6+0] and [r6+12], then exit ; Test r4 against 1; if true, write r5 to [r6+4] and [r6+16], then exit ; Test r4
...
```
**Status:** Medium ; Offsets and control flow are clearly visible in the disassembly ; Dual writes (primary/shadow pairs) are inferred from pattern, not documented ; The actual register layout and latency units (time vs. counts) are unknown without a hardware datasheet ; r5 type assumed uint32_t; it could be float or another encoding
