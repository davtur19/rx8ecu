...
In: r4: threshold value (interrupt priority level to compare against)  Out: r0: comparison result (1 if current SR priority > r4, else 0) ; SR: unchanged  Behavior: Load mask 0x00F0 (bits [7:4] = interrupt priority) into r5 ; Read SR into r0 with `stc sr,r0` ; AND r0 with mask (extract interrupt priority bits) ; Compare: if r0 > r4, continue; else skip to exit ; Return (rts) ; If comparison succeeded: load r4 into SR (but after rts, unreachable)
