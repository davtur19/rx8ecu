...
**Out:** Calls appropriate handler function FUN_00066650 with PID index ; Assembles OBD response packet in RAM ; Transmits response via txCAN_EventBased (0x000099B0) ; May set flags/status in response buffer
**Calls:** FUN_00066650 (0x00066650): Handler for matched PID (called with index r4) ; FUN_000674b6 (0x000674B6): Setup/encoding function (called with r4=r3, r5=48 or r0=120) ; thunk_FUN_00052aec (0x00067654): Dispatch/completion thunk ; txCAN_EventBased (0x000099B0): Transmit CAN response packet
Initialize: r5=1 (search count), r14=0 (loop counter), r13=FUN_00066650 ; PID table at 0xFFFFDC03, stride 2 bytes (1 byte per entry): ; Loop until matching PID found (r14 < r5): ; Load byte at
0xFFFFDC03 + r14*2 → r2 ; Compare r2 == r4 (requested PID): ; If match: call FUN_00066650(r14) → returns r0 ; Else: increment r14 and continue ; If match found: ; Jump to handler response assembly
(0x66AE2+) ; Handler path (if match): ; Load byte at 0xFFFFDC05 → r0 (status/flags) ; Test bit 6: if clear, return (exit) ; Set bit 4 (0x10) in status → r0 ; Store r0 to 0xFFFFDC05 ; Write response
...
(separator or padding) ; [4–7]: 0x00 (zeros / reserved) ; Call FUN_000674b6(r4=3, r5=48) → assembles data bytes ; Call thunk_FUN_00052aec(r4=127) ; Load response length/pointer from 0x0004BB04 ; Jump
