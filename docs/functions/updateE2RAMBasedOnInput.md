...
14, 15, 16, 20, 18, 19) ; cmd=9**: write 1 byte from addr r12 (index 12) ; cmd=10**: write 8 bytes from 0xFFFFC288 (index 2) ; cmd=11**: write 8 bytes from 0xFFFFC288 (index 2) then 1 byte from
0xFFFFC290 (index 10) ; cmd=14**: write 1 byte from 0xFFFFC292 (index 13) ; cmd=15**: write 1 byte from 0xFFFFC293 (index 15) ; cmd=0xFF**: write 1 byte from 0xFFFFC284 (index 0) then write 8 bytes
from 0xFFFFC288 (index 2) then 1 byte from 0xFFFFC290 (index 10) ; Default: no operation (bra to exit)
**Draft C:**
```c
