...
Check charging flag (0xC5DB): ; If not set: skip lookup and restore previous values; return ; Load alternator current (0xBBE8, fr4) and temperature offset (0x74A3C, fr3) ; Subtract baseline
temperature (0xA9FC, fr2) from offset ; Multiply by coefficient (0x74A40, fr1) to get adjusted temperature ; Compare adjusted temp against 0.0: ; If negative: use 0.0 ; Else: use adjusted temp ; Write
raw estimate to 0xB604 ; Apply first-order filter with tau=1e-05 to smooth transients ; Write filtered result to 0xB60C ; Load reference temperature contribution (0xAE40) and add to filtered result ;
Load saturation limits (0x74A48 lower, 0x74A44 upper) ; Saturate final value to [lower, upper] ; Write final battery temperature to 0xB610
**Draft C:**
```c
