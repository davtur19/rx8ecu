...
**Purpose:** Check if UDS/KWP2000 security access has been unlocked. Compare the session key or level against the current unlock state.
**Inputs:** r4: expected security level or session key (for example, 0x01 for level-1 unlock)
**Out:** r0: 0 if the security level matches the current unlock state (locked), 1 if mismatch (not unlocked) ; No global state changed; read-only access to the security flag
**Calls:** none
Load the global/RAM address 0xFFFFCFE4 into r1 (SecurityAccess state cell) ; Zero-extend r4 (extu.b r4,r4) – compare only the low byte ; Load the current unlock state from @r1 into r2, zero-extend (extu.b r2,r2)
...
**Uncertainties:** exact semantics: whether 0xFFFFCFE4 stores the active session level or an "unlocked level" flag ; whether multiple levels (0x01, 0x02, 0x03) are supported or just one bit ; expected values (0x00=locked, 0x01=level-1, or other?)
