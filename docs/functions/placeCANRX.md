# placeCANRX @ 0x9890
**Purpose:** Process a received CAN frame and place it into a message queue or buffer. It validates mailbox readiness before it reads RX data.
...
**Out:** r0: 1 if frame placed successfully, 0 if mailbox not ready or RX pending ; Data buffer at msg->@+8 filled with 8 bytes of CAN frame data ; HCAN mailbox registers read with getHCANRegisterAddress calls ; SR saved/restored; interrupts disabled during register access
