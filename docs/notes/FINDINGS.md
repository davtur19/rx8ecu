...
- vis_intake_control (0x23718): 3D lookup with RAM8[B33C/B33D/B33E] → desc 0x6AC60/0x6AC7C/0x6AC98/0x6ACB4 (all 21x18, type=8 u16, scale
  1/327.68); clamp [0,84] with 0x2404 (fpu_compare_and_select = clamp, NOT a comparator); 12-deep rolling history @RAM[FFFFB408..FFFFB43C];
...
- vfad_control_35BBC (0x35BBC): stock VFAD solenoid control, status word F754 bit 0x400 (mod's CAN_EmitLaunchStatus @0x57BE8 reads it as
  ...
  sm(0x5D800) → RAM8[C234] + F754 bit 0x400 (through 0x4BBC). 2026-08-01 audit: "launch_status_bit0400" was a mod-era misnomer — this IS stock
...
- `omp_control_task_1825E` (0x1825E) — OMP/waveform control task (756 B) — **LIFTED + emulator-verified** (c/omp_control_task_1825E.c,
  ...
  A968..A98B + 9ECD bit2 + CD06 + 78E35/36/37; writes ports 0xFFFF8078/807A/807C with complementary-encoded helpers (0x3EE58/0x3EE68: v =
  (b<<8)|~b).
...
- **Reset vector** @0x0000 = 0x8B8 `Manual_Reset`; initial SP @0x0004 = 0xFFFFDFA0. Manual_Reset → bsc_init (0x8CC) + gpio_init (0x8F6) →
  ...
  SP=[0xD9C8]=0xFFFF7304 with stack_frame_set_sp (0x4C7A = rts/mov r4,r15), then secondary_boot_main (0xA038), infinite loop.
- **App entry chain**: [0x1000]=0x12B4 → jsr 0x1038 secondary_boot_init (r4=0); 0x1038 checks [0x7FFFC]=0x2000 and [[0x7FFFC]] (ROM-ID
  ...
  jmp @r4 → 0xD49C. **Trampolines** 0x40 and 0x1094 identical: mov.l [lit],r15 (SP=0xFFFFDFA0) + jmp @r4; resetHandler exits with a jsr
...
- **resetHandler (0x4E0) verified** (matches c/reset_handler.c): bsr 0x572 (resetWatchdog?), hw_init_1/2/3 (0x170/0x41C/0x3D4), magic
  ...
  0x6C8, store magic, exit with 0x40.
...
- **0x6C8 boot continuation** = serial dispatch loop (flash/service bootloader): byte&0xF8 matches 0xA8→bsr 0x806, 0x88→jsr 0x4C, 0x90→jsr
  ...
  Handlers not traced.
- **0xD4B6 warm restart** (symbol main??, called from 0x64E0 RTOS task): bsr 0xD4FA (validate with 0x99C4 placeCANRX with DBCC table @0xDBCC
...
- **61550 common tail**: every mode (1/2/3) stores enc result @0xFFFFD6FC, status @0xFFFFD6FF; if w16@0xFFFFD700 == dtc →
  can_encode_handler_62ABC(dtc,0x20) (may update run-sum words 0xFFFF8E98/ 0xFFFF8E9A with 0x648B4); then
...
- **dtc_code_set/clear**: checksum-paired bytes (b,~b); set @0x46780 only when present-flag @0xFFFF8788 == 1 (readValue_8bit_ADDRESS_VAL
  default 1); both write 0 to state words 0xFFFF875C/0xFFFF875E with updateMemoryAtAddress_8bit.
...
- **0x18C5C**: wave(6) then A97B = 8. ✓. **0x18C6C**: A974>7: wave(3),A97B=0x10; A974==7: wave(4), A97B=4; A974<7: wave(2),A97B=0x10. ✓.
  **0x18C08**: A980==1: write16(0x807C,1) with 0x3EE58, diag-table store with 0x9668, A980=2; then A8F1==A974 → A97B=0x30,A980=1, else →
...
- **0x82xx/0x86xx mov.l r0,@(disp,Rm) / mov.l @(disp,Rm),r0**: register in bits 7-4, disp nib x4. GNU-as has NO syntax — re-encodes as
  0x1nmC/0x5nmC (for example 0x82C0 → 0x1C00). Rebuild forces `.word` (byte-exact by construction); they decode for inspection only.
...
- **Whole-ROM GNU-as round-trip**: every decoded word in both ROMs reassembles byte-exact (226,048/226,408 and 220,183/220,543); only
  non-round-trippable words are the 360/360 82xx/86xx, that canonicalize to 0x1nmC/0x5nmC.
...
- **Strings in-window are fragmentary**: 0x2000 holds '60E1D400' ROM-id and 'Copr.DENSO200' (the .word runs at 0x2002/0x2028 are mis-sliced
  ...
  Classifier requires a >=12-byte printable span, so random 4-char fragments (for example "OROb") are not strings.
...
- **CORRECTION (unblocks the low-SFR family):** `mov.w @(disp,PC)` SIGN-EXTENDS its literal (SH-2 semantics). Disassembler short comments
  (`; 0xCD49` / `; 0xF76C`) are truncated shorthand — with bit 15 set the real addresses are 0xFFFFCD49 / 0xFFFFF76C, and so on. So 0xCD4C, 0xD2C4,
  0xD2C5, 0xCE00/01, and so on, are actually 0xFFFFCD4C, 0xFFFFD2C4, 0xFFFFD2C5, 0xFFFFCE00/01 — all above mmap_min_addr=0x10000 and host-mmap-able
...
- **idx_table family (0x68780/0x6879C/0x687C8/0x687F4)**: 4 packed leaves over a RAM table at 0xFFFFD998, stride 0x46C, byte index; called
  with the 0x68776 wrapper (clear(0)) and function-pointer tables (pools 0x68CE0/0x695C0/0x695D0). clear = zero 3 words; step/step2 =
...
- **req_queue leaves (0x69602 store / 0x69694 clear)**: byte flag array at 0xFFFFDE38 + parallel long array at 0xFFFFDE40. store:
  ...
  function-pointer tables. The 0x69624..0x69692 dispatcher between them (7-entry loop: calls 0x3920, dispatches through the 0x69918 table to
...
- Caller logic (full disasm 0xDC8C..0xDD2B): resets byte@0xFFFFA40E=0; loop `while counter < 2`: base0 = (byte@0xB5A4==1)?0x4EA60:0x4EB60;
  ...
  paths).
- Callee facts: CANControllerSetup @0x9878 (writes on-chip MMIO 0xFFFFE400..0xFFFFE6FF, incl. cells canMessageSetup later checks, for example
  E402=0x803E, E404/E414 mailbox-derived); canMessageSetup @0x2B320 reads ALL its MMIO through the sign-extended HIGH aliases
...
- Stack convention (non-ABI, stack-passed, 2 x 32-bit result buffer): [r15+0]=out ptr, [r15+4]=a0, [r15+8]=a1; writes result[0]=low word and
  result[1]=high word with the ptr at [r15].
...
- Loop signature (verified instruction-for-instruction bit-exact, 0 mismatches on 100k+ vectors incl. all early-exit edges): 29-iteration
  ...
  2-bit-at-a-time radicand shift (shll/rotcl pairs on r2:r5), root bits accumulated in r1 with rotcl, final restore phase + sticky round-up
  (r1|=1 if remainder != 0).
...
- The SH-2 `mov.b/w/l Rm,@-Rn` family in `tools/sh2emu.py` decremented Rn BEFORE storing Rm, so when Rm == Rn (for example `mov.l r15,@-r15`
...
- Semantics (verified bit-exact vs emulated ROM, 100120 inputs incl. edge cases, 0 mismatches): exp = sext16(arg1); if exp>=0x7FFF ->
  ...
  reconstruct r0 = (arg2<<1)>>8 | (exp+0x7F)<<24 | bit31(arg2)<<31 with the shll/shlr/rotcr chain at 0x4880. The 0x483C-0x487E block is
...
- Stub-mechanics gotcha (repo first): 34-byte single-block stubs (0x11A34 pattern) OVERLAP for adjacent callees — getSR @0x3920 and setSR
  ...
  @r2; nop; [pad]; .long body` — pool lands at ((a+4)&~3)+4, that needs 2 pad bytes for a%4==0 targets and 0 for a%4==2) + a 36-byte shared
...
- **reset_handler @0x4E0 (re-verified with real ROM bytes)**: `c/tests/test_reset_handler_4E0.py` — real resetWatchdog @0x572 runs, hw-init
  ...
  there clobbers pool words the handler loads (mov.w 0x586,r2, and so on). Real watchdog writes word 0x5A1F -> 0xFFFFEC12 and 0xA53C -> 0xFFFFEC10
  (0xEC12/0xEC10 sign-extend with mov.w PC-relative).
- **OBD mode-01 PID getters @0x55E66 (getMAFOBD), 0x55E7C (getRPMOBD), 0x55EA2 (getSpeedOBD), 0x55EEA (getSTFTOBD), 0x55F02 (getLTFTOBD)** —
  ...
  a RAM sensor cell (16-bit sign-extended or 32-bit literal pointer) and clamp with floatToInt @0x24D0 to 0..255. Lift-correction (MAF):
...
- Table-address gotcha: the CAN TX table rows (for example 029B52 for can201, 029DC2 for can203) point one instruction PAST the prologue; the real
...
- **SendKey (0x58592-0x58610) is UNREACHABLE in 60E1D400**: entry 0x584B6-0x584BE routes only subfunc==1; whole-ROM branch scan shows
  0x58592 has exactly one incoming ref (bf/s @0x58516, unreachable). Subfunc != 1 → 0x5862C: subfunc==0 → resp with 0x55386; subfunc!=0 → NO
...
## 2026-08-06 — gen_c_lift_v8 multi-dispatch callers: missing-callee-lib root cause (bank 60E1D400)
- Swept ALL 28 multi-dispatch callers with v8 --emit after creating callee lib files. Result: **0 PASS**.
- f_*.c files only gate the EMIT check (missing-callee-lib). The v8 differential test (test_caller_*.py)
  RE-WALKS each callee from ROM via walk_v3(t, t+32) (tools/gen_c_lift_v8.py:776) — C file content is
  irrelevant to test PASS/FAIL.
- 18 callers using callee 0x60774 (a trampoline `bra 0x6085C`) now EMIT, but all MISMATCH: mirror RETs
  immediately at the call (target 0x6085C absent from CODE dict, beyond the t+32 span) while sh2emu runs
  the real body at 0x6085C. walk_v3 cannot model 0x6085C (rts at 0x6086A; +0x20 walk → None).
- 0x0295C4 (callees 0x29312+0x298F4): even with f_29312.c present, EMIT fails ('callee-walk', 0x29312) —
  walk_v3(0x29312, 0x29312+0x20) → None (rts at 0x29324, crosses into next function).
- 9 callers still fail 'missing-callee-lib' because their callees cannot be lifted by v7 --callee
  (compile-gate auto-deletes): 0x3934 (jmp @r6, op 0x462B unmappable), 0xAF10/0xC2E6/0xDD88/0x28148/
  0x2824E/0x28252/0x2BCA4/0x445A8 (sts.l pr,@-r15, op 0x4F22 unmappable), 0x4463E (label L_44668 error).
- Blockers are structural: v3 walk_v3 can't map sts.l-pr/jmp-@r6, can't follow trampolines past the
  hardcoded t+32 window, and diverges at rts crossings. Read-only tools cannot fix without changing
  walk_v3/v8 test semantics.

## 2026-08-06 — v7 literal-base LOAD fold is SELECTION-NEUTRAL; walker path-sensitivity bug fixed (v7.1)
- A/B (select_v3 + select_fpu, merged nospan cands, 60E0FC00, n=500 seed 0, empty outdir): HEAD vs CUR
  (with v7 fold) pools are BYTE-IDENTICAL — selected=84 (v3, no dedup) / 120 (fpu), rejected
  base_unresolved=513/822 unchanged. The v7 fold does NOT change selection on this candidate set; the
  earlier "1 new acceptance / base_unresolved 502→498" note was WRONG (artifact of c/ dir state).
- The v7 fold's real effect is in walk_v3 EMISSION: it unlocks the WALK of 0x5B100 and 0x5BADC
  (diagMeteringPumpPositionControl, uds_2f_iocontrol_entry_175c). At HEAD walk_v3 returns None for both
  → lift emitted but test SKIPPED (main() lines 3456-3459). With the v7 fold both walk ok (66 records).
- BUG exposed by the unlocked walk (pre-v7.1 state): walk_v3 baked a path-sensitive literal at
  0x5B194 as `r[4] = s8(_rdw(ram, 0xFFFFD171, 1))` → generated test case 52 FAIL
  (mirror=FFFFFFBD emu=FFFFFFE9). Root cause: the base chain r4=r6@0x5B190; r4+=0x14@0x5B192 uses
  r6+=0x14@0x5B17A which a forward bf@0x5B176 (taken on the failing path, T=0 → target 0x5B180) SKIPS.
  Runtime effective address is 0xFFFFD15D (byte 0xE9), linear-baked was 0xFFFFD171 (byte 0xBD).
- FIX (v7.1, gen_c_lift_v3.py emit_one mem block): extend `_dyn` with
  `(st['branches_seen'] and 'r%d'%base_reg not in st['lits'])` — a tracker-chain literal base (NOT a
  fresh pool load still in lits) after ANY admitted branch is emitted register-relative (dynbase). Pool
  loads (immutable, still in st['lits']) keep baking. The fold stays off for _dyn mems (dest UNKNOWN →
  a dependent mem drops the function — conservative).
- VERIFIED: regenerated 0x5B100 and 0x5BADC lifts+tests with the fix → both PASS 2000/2000.
  Emission at 0x5B194 is now `r[4] = s8(_rdw(ram, r[4], 1))` (register-relative). `make test` green;
  27 sampled c/tests (2 new + 25 random) all exit 0 (reset_handler_4E0 prints "OK ..." not "PASS").
- Cross-check with the pre-v7.1 file: 0x5B100 emits the baked literal (FAIL); git HEAD walk_v3 returns
   None (test skipped). Fix reproduces the empirically-verified dynamic form.

## 2026-08-06 — batch14 caller MISMATCH (A) root causes are mirror-fidelity, not the v7.1 fold; callee-walk load-chain relax (B, v8)
- (A) 18 batch14 callers failed `fold-literal MISMATCH` on D400 (60E1D400). Two representative cases
  reproduced: 0x007094 atu_control_bits_set (callee 0x4BBC) and 0x00A9F4 port_b_gpio_toggle_bit
  (callees 0x2054/0x2064). Root causes are NOT the v7.1 branches_seen/dynbase guard:
  1) Test-harness fidelity: in the emitted caller test mirror, op-stack (sts.l pr,@-r15) records
     updated only `sp`, leaving `r[15]` stale; the wrapper arg copy `mov r15,rN` then read a stale
     r[15] one push ahead (0xA9F4 case0: mirror r4=7829DA8B vs emu=000000F0). FIX: v8 template emits
...
`python3 tools/gen_c_lift_v8.py --emit <addr> --rom roms/stock/60E0FC00.bin` fails for
0x01F540, 0x001298, 0x0064BA, 0x006584, 0x011F20. Captured chains (decimal from tool, conv:
0x1F80E=129038, 0x1096=4246, 0x46=70, 0x310CC=200908, 0x1F81A=129050):

| caller | chain (hex) | unmapped |
|---|---|---|
| 0x01F540 | callee-lib 0x1F80E → callee-cfg → | 0x1F81A (0xFFFF) |
| 0x001298 | 0x1038 → 0x1090 → | 0x1096 (0x0000) |
| 0x0064BA | 0xD21E → 0x4E0 → 0x40 → | 0x46 (0x0000) |
| 0x006584 | 0x1182A → 0x21BB2 → 0x30C7C → 0x310AA → | 0x310CC (0xFFFF) |
| 0x011F20 | 0x669CE → 0x67286 → 0x67660(thunk 0x52B22) → 0x52B22 → 0x546FA → 0x547F0 → 0x1038 → 0x1090 → | 0x1096 (0x0000) |

**Group A — 16-bit RAM-window jsr target misread as ROM callee (0x1096 ×2, 0x46).**
`mov.w @(disp,pc),rN` loads a 16-bit RAM-address constant from the caller's trailing pool, then
`jsr @rN`; build_cfg resolves the literal as a static ROM callee and `_walk_callee` walks ROM
bytes that are not code. reset_ZERO 0x1038: `0x1070 mov.w @0x1084,r3 (=0x1090); 0x1072 jsr @r3`.
main_init 0x4E0: `0x0568 mov.w @0x594,r3 (=0x0040); 0x056A jsr @r3`. 0x40 and 0x1090 are
IDENTICAL 12-byte blocks `DF01 442B 0009 0000 FFFF DFA0` = reset stub (`mov.l @0x48,r15` SP=0xFFFFDFA0;
`jmp @r4`; nop; then 0x0000 padding). Walk hits 0x0000 (invalid opcode) at 0x1096 / 0x46 →
('unmapped', pc). Sibling RAM-pool jsr targets that don't fail (0x1364, 0x12A2) only survive
because their bytes decode to valid ops by luck.

**Group P — post-rts literal-pool continuation decodes 0xBxxx pool words as `bsr` (0x1F81A, 0x310CC).**
After the first delayed rts, the linear walk keeps running into the function's trailing literal
pool (seen_rts path, gen_c_lift_v8.py:1316-1326). Pool words that are valid SH-2 opcodes still
emit records — 16-bit RAM/table addresses 0xB27C/0xB160/0xB162/0xB1EA (pool of lambda_rich_lookup_store
0x1F540, at 0x1F5A2..0x1F5AA) and 0xB120/0xBF3*/0xB17C/0xB284 (pool of getClosedLoopTarget? 0x30C7C,
end 0x30DE4, at 0x30DA6..0x30DB4) decode as `bsr`. Spurious callees: 0x1F80E (gap 0x1F756-0x1F890
= data/table region; walk dives into 0xFFFFB1xx far-ptr table → 0xFFFF at 0x1F81A) and 0x310AA
(4-byte dispatch table base; entries 0xFFFFBFxx RAM + 0x00077xxx → f32_tbl_data_770CD; walk decodes
table as opcodes until 0xFFFF at 0x310CC).
