...
Previous session's "~74 pool_v5 candidates" does NOT reproduce against current catalog + lifted-state. Faithful re-scan (`tmp/analyze_*`):
...
- 626-vs-74 gap: v3 selector counted no-span candidates dropped entirely in v4.
- To lift a composable caller, callee must be re-generated as NEW `c/lib/f_<hex>.c` (state-struct ABI); most callees nest calls → recursive composition (depth guard).
...
## Blockers / scope
- callee-struct/C emission + merged-mirror test large; implemented only for tail-call leaf cohort. Full recursive jsr/bsr composition = next milestone.
...
- **ST-ABI callee emission** (`c/lib/f_<hex>.c`, `void f_X(ST *s)`): re-emits any call-free span from the v3 walker, register/system tokens → `s->r[N]`/`s->pr`/`s->T`; compile-gated (`cc -O2 -c`, delete on failure). Verified vs original lift ABI over **5000 random inputs: identical r0 + memory (seed 42)** for `f_3EE58`.
...
- **Differential test** (`--compose-test 0xADDR`): flattened caller+callee CODE, nested-return mirror (`call`→pc=target; `rts`→pc=pr; exit when pc leaves CODE) vs sh2emu oracle. **28/32 tail-dispatch callers PASS 500/500, 0 skipped**; 4 declined (multi-dispatch: 2 jmp sites w/ conditional branch to 2nd — needs full CFG).
