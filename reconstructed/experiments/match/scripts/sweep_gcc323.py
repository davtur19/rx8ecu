#!/usr/bin/env python3
"""
sweep_gcc323.py — full matrix sweep for the match-and-compile experiment using
GCC 3.2.3 sh-elf (xgcc from /home/davide/gcc323-build), the era of the RX-8 ROM.

Key differences vs the 3.4.6 harnesses:
  - GCC 3.2.3 has NO -m2e (cc1 rejects "2e"); SH-2 core without the single FPU
    is selected with plain **-m2** (same as 3.3.6).
  - GCC 3.2.3 has NO -fno-if-conversion / -fno-if-conversion2 (cc1: unrecognized
    option) — unlike 3.3.6/3.4.6.  The flag matrix below therefore drops those
    knobs and uses only the -f* switches that exist in 3.2.3 plus the
    -m1/-m3/-m3e/-m4-single-only/-m4-nofpu CPU switches from TARGET_SWITCHES.
  - GCC 3.2.3 DOES accept -mhitachi/-mrelax/-mspace/-misize/-mnomacsave
    (TARGET_SWITCHES in gcc-3.2.3/gcc/config/sh/sh.h lines 270-316).

Pipeline: gcc 3.2.3 -S -> /usr/bin/sh-elf-as (-isa=sh2e; superset, accepts all
SH-2 integer insns) -> sh-elf-objcopy --only-section=.text -> byte compare
against the exact ROM window (offset-relative), same as sweep_gcc336.py.

Usage: python3 scripts/sweep_gcc323.py [--out /tmp/sweep_gcc323/report.txt]
...
STUB  = "/tmp/stubinc"
XGCC  = "/home/davide/gcc323-build/gcc/xgcc"
XB    = "/home/davide/gcc323-build/gcc/"
...
# excluded) and sweep_gcc336.py for the saturating-add family (body+pool).
CASES = [
...
# (name, flags) — -m2 (base SH-2 core) is prepended by the harness.
# NOTE: -fno-if-conversion / -fno-if-conversion2 DO NOT exist in 3.2.3
# (cc1: unrecognized option), so the movt-killing recipe of 3.4.6/3.3.6 is
...
FLAGSETS = [
    ...
    ("O2.nodel",               ["-O2", "-fomit-frame-pointer", "-fno-delayed-branch"]),
    ...
    ("O1.omitfp.unrollall",    ["-O1", "-fomit-frame-pointer", "-funroll-all-loops"]),
    ("O1.nodel.unroll",        ["-O1", "-fomit-frame-pointer", "-fno-delayed-branch", "-funroll-loops"]),
    ("m1.O1.omitfp",           ["-m1", "-O1", "-fomit-frame-pointer"]),
    ("m1.O1.nodel",            ["-m1", "-O1", "-fomit-frame-pointer", "-fno-delayed-branch"]),
    ("m3.O1.omitfp",           ["-m3", "-O1", "-fomit-frame-pointer"]),
    ("m3.O2.omitfp",           ["-m3", "-O2", "-fomit-frame-pointer"]),
    ("m3e.O1.omitfp",          ["-m3e", "-O1", "-fomit-frame-pointer"]),
    ("m4singleonly.O1.omitfp", ["-m4-single-only", "-O1", "-fomit-frame-pointer"]),
    ("m4nofpu.O2.omitfp",      ["-m4-nofpu", "-O2", "-fomit-frame-pointer"]),
    ("m4nofpu.O1.omitfp",      ["-m4-nofpu", "-O1", "-fomit-frame-pointer"]),
    ("m1.O2.omitfp",           ["-m1", "-O2", "-fomit-frame-pointer"]),
]
...
def main():
    ...
    lines = []
    lines.append(f"# sweep_gcc323  gcc={XGCC}  version={ver}  target={target}")
    lines.append(f"# base ISA = -m2 (GCC 3.2.3 has NO -m2e; SH-2 core, no single FPU)")
    lines.append("# NO -fno-if-conversion / -fno-if-conversion2 in 3.2.3 (unrecognized option) -> movt-kill recipe unavailable")
    ...
    lines.append("# === BEST PER SOURCE (GCC 3.2.3, base -m2) ===")
