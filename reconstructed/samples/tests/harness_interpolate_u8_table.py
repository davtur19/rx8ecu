#!/usr/bin/env python3
"""
harness_interpolate_u8_table.py — equivalence of rx8_interpolate_u8_table @0x26B0.

Reconstructed source: samples/src/rx8_interpolate_u8_table.c
Verified lift   : c/interp_leaves.c (interpolate_uint8Table @ 0x26B0)
...
with that helper's results still live in registers:
    ...
    in:  r0 = cell index i, r1 = u8-cell-array pointer, fr0 = t in [0,1)
...
Procedure (Track-A pattern):
  ...
  2. edge vectors (every index with t=0.0 incl. the clamp-high i=n-1 case,
     ...
     (i, t) pairs, over real u8 cell arrays read from ROM map descriptors of
     the same 60E1D400.bin plus one synthetic extremes array,
  3. run the ROM bytes @0x26B0 in tools/sh2emu.py on the same vectors,
  4. run the host C on the same vectors,
  5. compare the raw float bits — 0 mismatches required.

Usage:  python3 harness_interpolate_u8_table.py [N]   (default N = 20000)
"""
import os
import struct
import subprocess
...
# Real u8-cell arrays from 1-D map descriptors in THIS ROM (60E1D400.bin),
# found with tools/mapscan.py's descriptor layout (u16 count; +8 values ptr).
# values@0x6D59C  6 cells [90,90,125,160,160,160]   RPM -> u8 map
# values@0x6E6F0  7 cells [175,175,175,190,190,200,215]  temp -> u8 map
# values@0x6D114 10 cells [0,0,22,22,22,22,22,36,36,36]
# values@0x6D138  6 cells [44,44,44,44,0,0]
DESCRIPTORS = (0x69A54, 0x69C4C, 0x69970, 0x69984)

# Synthetic extremes table: exercises both uint8 endpoints and steep deltas.
SYNTHETIC = [0x00, 0xFF, 0x80, 0x01, 0xFE, 0x7F, 0x40, 0xBF, 0x00, 0xFF]

CELL_BASE = 0x30000000            # sparse-RAM address backing the cell array

SAMPLES = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
BUILD_DIR = os.path.join('/tmp', 'rx8-recon-interpolate_u8_table')


class SH2E(SH2):
    """SH2 + call_leaf(): inject arbitrary initial registers (r0-r15, fr0-fr15),
    needed for interpolate_uint8Table's r0/r1/fr0 -> fr2 leaf-level convention
...
def build_oracle(cc='cc'):
    ...
    cmd = [cc, '-O2', '-Wall', '-Wextra',
           ...
           os.path.join(SAMPLES, 'tests', 'oracle_interpolate_u8_table.c'),
           os.path.join(SAMPLES, 'src', 'rx8_interpolate_u8_table.c'),
...
def load_u8_table(cpu, desc_addr):
    """Read count + u8 values array of a 1-D map descriptor straight from the ROM."""
    rom = cpu.rom
    count = struct.unpack_from('>H', rom, desc_addr)[0]
    vp = struct.unpack_from('>I', rom, desc_addr + 8)[0]
    return list(rom[vp:vp + count])
...
def main():
    ...
    tables = [load_u8_table(cpu, d) for d in DESCRIPTORS] + [SYNTHETIC]
    ...
    for i, t, cells in vectors:
        ram = {CELL_BASE + k: c & 0xFF for k, c in enumerate(cells)}
    ...
    lines = ['u8 %X %08X %X %s' % (i, f2bits(t), len(cells),
    ...
    report('interpolate_uint8Table', ADDR, n, mismatches, edges=len(vectors))
