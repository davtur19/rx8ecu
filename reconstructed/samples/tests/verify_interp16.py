#!/usr/bin/env python3
"""
verify_interp16.py — era-ROM toolchain (gcc 3.4.6) validation of Lotto 2:
rx8_interpolate_u16_table @0x26D0.

Closes the "ROM -> abstract C -> era-ROM toolchain (gcc 3.4.6)" loop on the
*behavioural* plane for the UINT16-cell linear-interpolation leaf, using the
same proven pattern as verify_gcc346.py (which validated the Lotto-1 pure
family): build with the archived sh-elf gcc 3.4.6, link at 0x4000, objcopy
the .text blob, load the blob into tools/sh2emu.py through the sparse ram
overlay, and compare it instruction-for-instruction against the real ROM bytes
at 0x26D0 on the same input vectors.

Calling convention (deduced from the ROM disassembly @0x26D0, NOT from the
host prototype — this is exactly the non-ABI leaf-style of the u8 sibling
@0x26B0):

    ROM  in: r0 = cell index i (already found by axis-search), r1 = pointer to
              the u16 cell array, fr0 = t in [0,1)
...
(in /tmp/verify_interp16/work/fmaf_shim.c, never committed) that implements
`a*b+c`.  gcc 3.4.6 recognises that the sh-2 has the 'fmac' instruction and
compiles this shim itself to a single `fmac` (see blob disassembly @0x4038),
so the shim is genuinely single-rounded and bit-exact with the ROM path — no
...
over several u16 cell arrays (all-zero, all-0xFFFF, a synthetic extremes/
pattern array and two real ROM descriptor tables), then padded to N = 3000
with seeded-random (i in-range, t in {0, 0.5, 1.0}).
...
Comparison: ROM fr2 bits vs blob fr0 bits — the raw IEEE single-precision
bit patterns must match for 0 mismatch to pass.

Read-only w.r.t. the repo: everything it writes goes to /tmp.  Exit code 0 iff
0 mismatches.

Usage:  python3 verify_interp16.py [N]   (default N = 3000)
"""
import os
import struct
import subprocess
...
STUB_INC = '/tmp/verify_interp16/inc'      # stub stdint.h / math.h (never committed)
WORK = '/tmp/verify_interp16/work'         # objects / ls / blobs / shim
...
CELL_BASE = 0x2000                         # the u16 table backing region

ROM_ADDR = 0x26D0                          # the real ROM bytes @60E1D400.bin
ENTRY = 'rx8_interpolate_u16_table'        # blob symbol (gcc ABI)
N_DEFAULT = 3000
...
class SH2L(SH2):
    ...
    required for the interp-u16 leaf's non-ABI entry (r0/r1/fr0 -> fr2), which
    cpu.call() cannot seed.  Line-for-line copy of SH2.call()'s body (the rosetta
    used by harness_interpolate_u16_table.py and c/tests/test_interp_leaves.py).
    """
...
def build_blob():
    ...
    subprocess.run(
        [XGCC, '-B', XGCC_B, '-m2e', '-O1', '-fomit-frame-pointer',
         '-c', os.path.join(SRC_DIR, 'rx8_interpolate_u16_table.c'),
...
# ============================================================================
# Real u16-cell arrays from 1-D map descriptors read from 60E1D400.bin
# (values pointer at desc+8, u16 count at desc+0) — reused from the harness.
DESCRIPTORS = (0x699BC, 0x69BC0)

# Synthetic extremes / pattern arrays: both uint16 endpoints, steep deltas,
# plus all-0 and all-0xFFFF degenerate tables.
PATTERN = [0x0000, 0xFFFF, 0x8000, 0x0001, 0xFFFE, 0x7FFF, 0x4000, 0xBFFF,
           0x0000, 0xFFFF, 0x1234, 0xDEAD, 0x5678, 0x9ABC]
ALLZ = [0x0000] * 7
ALLF = [0xFFFF] * 7
...
def real_tables(cpu):
    ...
    for d in DESCRIPTORS:
        ...
        cells = [struct.unpack_from('>H', rom, vp + i * 2)[0]
...
def main():
    ...
    total = len(mism)
    if total == 0:
        print('verify_interp16: ROM @0x%X vs gcc-3.4.6 blob @0x%X '
              '(entry 0x%X), %d vectors, 0 mismatches — OK'
              % (ROM_ADDR, LINK_BASE, entry, n))
    else:
        print('verify_interp16: %d vectors, %d mismatches — FAIL'
              % (n, total))
        for s in mism:
            print('    ' + s)
