#!/usr/bin/env python3
"""
harness_knock_related_init.py — equivalence of rx8_knock_related_init @0xC3C8.

Reconstructed source: samples/src/rx8_knock_related_init.c
Verified lift   : c/knockRelatedInit.c (knockRelatedInit; lift header labels
                  it "@0xC1F8 (60E0FC00) / 0xC3C8 (60E1D400)" — the harness
                  drives the emulator at 0xC3C8 because the ROM under test,
                  60E1D400.bin, holds the function there, reached by
                  `bsr 0xC3C8` from knockFunctionInit @0xC31C; 0xC1F8 in
                  60E1D400.bin is unrelated code.  ROM wins over the brief).
...
harness_knock_function_init.py):
  ...
  - emulator side: seed the 19 written cells + 13 store-boundary sentinels
    in the sparse `ram` overlay, call the ROM entry @0xC3C8 (self-contained;
...
The critical properties this harness pins:
  ...
  - the lift's "copy raw ADC" / "RPM ref" reads are FALSE: the ROM reads NO
    RAM input at all, so any oracle variant that reads 0xFFFF9F0E /
    0xFFFF9F80 fails every vector;
  - the per-rotor B-pair is NOT zeroed: 0xFFFFA350/0xFFFFA354 = 10.0 and
    0xFFFFA368/0xFFFFA36C = 64.0.

Usage:  python3 harness_knock_related_init.py [N]  (default N = 20000)
"""
import os
import struct
import subprocess
...
ADDR = 0xC3C8          # 60E1D400.bin (see header: NOT 0xC1F8, which is the
                       # 60E0FC00.bin address / unrelated code here)
N_DEFAULT = 20000
SEED = 0x60E1D400
BUILD_DIR = '/tmp/rx8-recon-knock_related_init'

# ---- RAM cells written by the function (see rx8_knock_related_init.c) ----
...
def build_oracle():
    ...
    cmd = ['cc', '-O2', '-Wall', '-Wextra',
           ...
           os.path.join(samples, 'tests', 'oracle_knock_related_init.c'),
           os.path.join(samples, 'src', 'rx8_knock_related_init.c'),
           '-lm', '-o', oracle]
...
def main():
    ...
    #     call the actual ROM bytes @0xC3C8, read the cells back.
    emu = []
    for v in vectors:
    ...
    report('knock_related_init', ADDR, n, mismatches, edges=len(edges))
