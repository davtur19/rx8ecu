/* ============================================================================
 * oracle_div32_signed.c  —  host test rig for rx8_div32_signed @0x3FE8
 * ============================================================================
 ...
 * division logic — that lives solely in src/rx8_div32_signed.c.  The op token
...
int main(void)
{
    ...
    while (fgets(line, sizeof line, stdin)) {
        ...
        if (sscanf(line, "div %lx %lx", &divisor, &dividend) == 2) {
            printf("%08lX\n",
                   (unsigned long)(uint32_t)rx8_div32_signed(
                       (int32_t)(uint32_t)divisor,
                       (int32_t)(uint32_t)dividend));
        } else {
