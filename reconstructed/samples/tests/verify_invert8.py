#!/usr/bin/env python3
"""
verify_invert8.py — era-ROM toolchain (sh-elf gcc 3.4.6) validation of
rx8_invert_and_return_8bit @0x2044 — the 8-BIT complement-residual reader.

Target
------
    mov.w @r4,r3   ; r3 = 16-bit big-endian cell (hi<<8|lo), sign-extended
    mov    r3,r0
    shlr8  r3      ; r3 = hi
    not    r0,r0   ; r0 = ~value (low byte = ~lo)
    sub    r3,r0   ; r0 = ~value - hi
    rts
    extu.b r0,r0   ;  (delay slot) return low byte: (~lo - hi) & 0xFF

Modulo 256, `~lo - hi == ~(hi + lo)`, so the function is the ones'-complement
residual of the (value, ~value) byte pair — 0 iff the cell is self-consistent.

POINTER CONVENTION on BOTH sides (unlike verify_checksum.py).
The ROM reads its operand through the pointer in r4 (`mov.w @r4,r3`), and the
reconstructed C (`uint8_t f(const uint8_t *addr)`) dereferences r4 too.  There
is therefore NO by-copy/by-pointer split here: both sides place the big-endian
(hi,lo) cell in the emulated RAM overlay and receive r4 = &cell.  Both perform
exactly one 2-byte read and no writes.

Signature verified:  uint8_t f(const uint8_t *cell_r4) -> r0, leaf, no writes.

What this harness does
----------------------
  (a) compiles rx8_invert_and_return_8bit.c with the era-ROM recipe
      `-m2e -O1 -fomit-frame-pointer` via /home/davide/gcc346-build/gcc/xgcc,
      links it at 0x4000 with sh-elf-ld and extracts .text,
  (b) drives the ROM bytes @0x2044 and the blob @0x4000 on identical vectors,
      each carrying a full RAM image:
        - the 2-byte cell at a deterministically varying address
  ...
  (c) compares r0 AND the complete post-call RAM state (the ROM leaf performs
      exactly one 2-byte read and no writes, so RAM must be untouched on both
      sides — this also proves the blob does not secretly read/write memory),
  (d) as a third leg, checks the host-C oracle (oracle_invert_and_return_8bit.c)
      against the blob.

Read-only w.r.t. the repo: everything written goes to /tmp/verify_invert8
(separate from verify_gcc346.py's /tmp/verify_gcc346 and verify_checksum.py's
/tmp/verify_checksum); exit non-zero iff any mismatch.  Does NOT modify
verify_gcc346.py, README.md, Makefile or tools/.

Usage:  python3 tests/verify_invert8.py [N]   (default N = 3000 random cells)
"""
import os
import subprocess
import sys
...
STUB_INC = '/tmp/verify_gcc346/inc'        # stub stdint.h/math.h (shared, recreated if missing)
WORK = '/tmp/verify_invert8'               # own workdir (never touches gcc346's)
...
ROM_ADDR = 0x2044                       # rx8_invert_and_return_8bit
ENTRY_SYM = 'rx8_invert_and_return_8bit'
...
N_DEFAULT = 3000                        # seeded random cell values (>= 3000)
SEED = 0x2044
EDGE_VALS = [
    0x0000, 0xFFFF, 0x0001,             # 8-bit boundaries
    0x00FF, 0xFF00, 0x8080,             # hi/lo extremes
    0x7F80, 0x807F,                     # exact complements (residual 0)
    0x1234, 0xABCD, 0xDEAD, 0xBEEF,     # dense bit patterns
]
...
def expected(val):
    """The residual both sides must return: ~(hi + lo) & 0xFF."""
    hi = (val >> 8) & 0xFF
    lo = val & 0xFF
    return (0xFF - ((hi + lo) & 0xFF)) & 0xFF


def build_ram(cell_addr, size, align, val, pat):
    """RAM image: guards + size-byte buffer + the 2 big-endian cell bytes at
    cell_addr (may spill past the buffer into the post guard for size < 2)."""
    ram = {}
    ...
    for i in range(2):                                  # the cell, big-endian
        ram[cell_addr + i] = (val >> (8 * (1 - i))) & 0xFF
    return ram
...
def gen_vectors(n):
    ...
    def add(val, size, align, pat, desc):
        vecs.append({'val': val, 'size': size, 'align': align,
                     'pat': pat, 'addr': CELL_BASE + align,
                     'exp': expected(val), 'desc': desc})

    for v in EDGE_VALS:
        add(v, 4, 0, 'zero', 'edge val=0x%04X' % v)
    for pat in PATTERNS:                                # full deterministic matrix
        for size in SIZES:
            for align in ALIGNS:
                val = rng.getrandbits(16)
                add(val, size, align, pat,
                    'pat=%-4s size=%-3d align=%d val=0x%04X' % (pat, size, align, val))
    for i in range(n):                                  # seeded random stream
        ...
        add(val, size, align, pat,
            'rnd#%d val=0x%04X size=%d align=%d pat=%s'
...
def ensure_stubs():
    """Rebuild the minimal stdint.h/math.h stubs if /tmp/verify_gcc346/inc is
    missing (the archived gcc 3.4.6 was configured --without-headers)."""
    if not os.path.exists(STUB_INC):
        os.makedirs(STUB_INC, exist_ok=True)
        _write_stubs()
    elif not os.path.exists(os.path.join(STUB_INC, 'stdint.h')):
        _write_stubs()


def _write_stubs():
    with open(os.path.join(STUB_INC, 'stdint.h'), 'w') as f:
        f.write('#ifndef _STDINT_H\n#define _STDINT_H\n'
...
def build_blob():
    ...
    obj, elf, blb = (os.path.join(WORK, 'inv8.' + e)
                     for e in ('o', 'elf', 'bin'))
    subprocess.run(
        [XGCC, '-B', XGCC_B, '-m2e', '-O1', '-fomit-frame-pointer',
         '-c', os.path.join(SRC_DIR, 'rx8_invert_and_return_8bit.c'),
...
def build_host_oracle():
    """Host-C oracle: oracle_invert_and_return_8bit.c + reconstructed src."""
    binp = os.path.join(WORK, 'oracle_invert8')
    cmd = [CC_HOST, '-O2', '-Wall', '-Wextra',
           '-I', INC_DIR, '-I', SRC_DIR,
           os.path.join(TESTS, 'oracle_invert_and_return_8bit.c'),
           os.path.join(SRC_DIR, 'rx8_invert_and_return_8bit.c'),
           '-o', binp]
...
def run_pair(cpu, entry, base, vec):
    ...
    POINTER VERSION on both sides: r4 = &cell (the 2-byte (hi,lo) pair), both
    sides read it through r4 and perform no writes.
    Returns (rom_r0, blb_r0, pre_ram, rom_post, overlay, blb_post)."""
    ram = build_ram(vec['addr'], vec['size'], vec['align'], vec['val'], vec['pat'])

    rom_r0 = cpu.call(ROM_ADDR, r4=vec['addr'], ram=dict(ram))
    rom_post = dict(cpu.ram)

    overlay = dict(base)                # blob code at LINK_BASE
    overlay.update(ram)                 # + cell / guards at 0x3000
    blb_r0 = cpu.call(entry, r4=vec['addr'], ram=overlay)
...
def main():
    ...
    if ENTRY_SYM not in syms:
        sys.exit('verify_invert8: entry symbol %r not found in blob '
    ...
    for v in vecs:
        ...
        if rom_r0 != blb_r0:
            rb += 1
            if len(samples) < 5:
                samples.append('r0  %s ROM=0x%02X blob=0x%02X (exp=0x%02X)'
                               % (v['desc'], rom_r0, blb_r0, v['exp']))
        if rom_post != pre:
            rom_ram_bad += 1
            if len(samples) < 5:
    ...
    lines = ['u8 %02X %02X' % ((v['val'] >> 8) & 0xFF, v['val'] & 0xFF)
             for v in vecs]
        ...
        for i, (h, o) in enumerate(zip(blb_r0s, outs)):
            if h != int(o, 16):
                ...
                if ob == 1 and len(samples) < 5:
                    samples.append('oracle-vs-blob: blob=0x%02X host=%s (vec#%d)'
                                   % (h, o, i))
    ...
    print('%s rx8_invert_and_return_8bit @0x%X  (ptr r4 -> r0, leaf, no writes)'
          % (status, ROM_ADDR))
    print('    signature found: uint8_t f(const uint8_t *cell_r4) -> r0'
          '  [blob: same pointer convention, reads (hi,lo) cell]')
    ...
    if bad:
        print('verify_invert8: %d mismatch(es) total — FAIL' % bad)
        sys.exit(1)
    print('verify_invert8: all OK (0 mismatch; post-call RAM untouched on both sides)')
