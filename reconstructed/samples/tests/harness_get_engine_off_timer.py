#!/usr/bin/env python3
"""
harness_get_engine_off_timer.py — equivalence of
rx8_get_engine_off_timer @0x3279E.

Reconstructed source: samples/src/rx8_get_engine_off_timer.c
Verified lift   : c/getEngineOffTimer.c (same address, same behaviour; also
                  c/add16bitSaturate.c for the 0x2460 leaf).
...
It acts on fixed RAM cells (u8 engine-running flag @0xFFFFA41C, u16 engine-off
timer @0xFFFFBFD6), so the equivalence check compares the RAM side-effect, not
a return value:

  - emulator side: seed the two cells in the sparse ram overlay, call the ROM
    entry @0x3279E (which internally jsr's the real add16bitSaturate leaf
    @0x2460), read the timer word back;
  - host side: the dedicated oracle mmap()s the pages backing the cells, seeds
    the same numeric bytes, runs the reconstructed C, reads them back.

Procedure (Track-A pattern):
  1. build the dedicated oracle (tests/oracle_get_engine_off_timer.c
     + samples/src/rx8_get_engine_off_timer.c) with system gcc;
  ...
  3. run the ROM bytes @0x3279E in tools/sh2emu.py on the same pre-states;
  4. run the host C on the same pre-states;
  5. compare the post-state timer words — 0 mismatches required.

Usage:  python3 harness_get_engine_off_timer.py [N]
        (default N = 20000; reduce to 5000 if the host is slow)
"""
import os
import subprocess
import sys
...
ADDR = 0x3279E
FLAG_ADDR = 0xFFFFA41C          # u8  engine-running flag (ROM: mov.w+mov.b+extu.b)
TIMER_ADDR = 0xFFFFBFD6         # u16 engine-off timer     (ROM: mov.l addr + mov.w)
N_DEFAULT = 20000
BUILD_DIR = '/tmp/rx8-recon-get_engine_off_timer'

# Edge pre-states.  The flag matters only as == 1 (0x00/0x02/0x7F/0x80/0xFE/0xFF
# must ZERO the timer; 0x80 pins the mov.b sign-extend + extu.b path).  The
...
def build_oracle(cc='cc'):
    ...
    cmd = [cc, '-O2', '-Wall', '-Wextra',
           ...
           os.path.join(SAMPLES, 'tests', 'oracle_get_engine_off_timer.c'),
           os.path.join(SAMPLES, 'src', 'rx8_get_engine_off_timer.c'),
           '-o', oracle]
...
def main():
    ...
    lines = ['oft %02X %04X' % (flag, timer) for flag, timer in vectors]
    ...
    report('getEngineOffTimer', ADDR, n, mismatches, edges=len(EDGE))
