/* ============================================================================
 * oracle_interpolate_u16_table.c — host test rig for rx8_interpolate_u16_table
 * ============================================================================
 * Compile together with src/rx8_interpolate_u16_table.c and pipe test vectors
 * on stdin; one vector per line, whitespace-separated hex tokens:
 *
 *     u16 <i> <t_bits> <n> <c0> <c1> ... <c{n-1}>      -> <result_bits>
 *
 ...
 *   c0..     : the n cell values (uint16)
 *
 * The result is printed as the raw bits of the returned float (fr2 on the ROM).
 *
 * NOTE: rx8_interpolate_u16_table is declared here rather than in rx8_samples.h
 * (which is off-limits for this task) — the reconstructed name maps to the ROM
 * leaf at 0x26D0 (see rx8_interpolate_u16_table.c).
 * ==========================================================================*/
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
...
/* 0x26D0 — u16-cell linear-interpolation leaf (see rx8_interpolate_u16_table.c). */
...
int main(void)
{
    ...
    while (fgets(line, sizeof line, stdin)) {
        ...
        for (k = 0; k < n; k++) {
            cells[k] = (uint16_t)strtoul(strtok(NULL, " \t\r\n"), NULL, 16);
        }

        i_tbits = (uint32_t)tbits;
        memcpy(&t, &i_tbits, sizeof t);     /* exact float from raw bits */

        result = rx8_interpolate_u16_table((int32_t)i, cells, t);
