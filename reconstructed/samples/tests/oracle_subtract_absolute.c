/* ============================================================================
 * oracle_subtract_absolute.c — host test rig for rx8_subtract_absolute @0x23DC
 * ============================================================================
 * Compile together with samples/src/rx8_subtract_absolute.c and pipe test
...
int main(void)
{
    ...
    while (fgets(line, sizeof line, stdin)) {
        unsigned long a, b;
        if (sscanf(line, "abs %lx %lx", &a, &b) == 2) {
            float out = rx8_subtract_absolute(b2f((uint32_t)a),
                                              b2f((uint32_t)b));
            printf("%08X\n", f2b(out));
        } else {
