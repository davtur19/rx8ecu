/*
 * =============================================================================
 * rx8_interpolate_u8_table.c  —  UINT8-TABLE LINEAR-INTERPOLATION LEAF
 * =============================================================================
 * ROM         : roms/stock/60E1D400.bin (Mazda RX-8 PCM, N3J1 family)
 * Address     : 0x26B0
 * Status      : VERIFIED — behavioural equivalence to the ROM bytes held by
 *               reconstructed/samples/tests/harness_interpolate_u8_table.py
 *               (host-gcc vs tools/sh2emu.py over 20000 random + edge vectors,
 *               real u8 cell arrays from ROM map descriptors plus synthetic
 *               extremes; 0 mismatches).
 * Lift (truth): c/interp_leaves.c  (interpolate_uint8Table @ 0x26B0)
 *
 * WHY THIS FUNCTION EXISTS
 * ------------------------
 * The typed-cell 1-D interpolation primitives that TwoDLookup (2DLookup.c)
 * dispatches to via its type jump table — this leaf handles type 4 (u8 cells) —
 * and that the 2-D bilinear FP lookups build each row from internally
 * (3dLookup.c's row-bilinear helper @0x25C8 calls it once per row).
 *
 ...
 *     r1  = pointer to the u8 cell array (base of the descriptor's values)
 ...
 *
 * ROM BODY (disassembled 60E1D400.bin @ 0x26B0)
 * ----------------------------------------------
 *     fldi0    fr2           ; fr2 = 0.0
 *     add      r0,r1         ; r1 = cells + i
 *     mov.b    @r1+,r0       ; r0 = cell[i] (sign-extending load)
 *     fcmp/eq  fr0,fr2       ; T = (t == 0.0)
 *     extu.b   r0,r0         ; zero-extend -> unsigned 8-bit value
 *     lds      r0,fpul
 *     bt/s     ret           ; t == 0.0 -> return cell[i]
 *     float    fpul,fr2      ;   (delay slot: fr2 = (float)cell[i], runs ALWAYS)
 *     mov.b    @r1,r0        ; r0 = cell[i+1]
 *     extu.b   r0,r0         ; unsigned again
 *     lds      r0,fpul
 ...
 * type 4 -> handler 0x26B0, u8 cells, no scale/offset). */
#define RX8_INTERP_TYPE_U8 4u
...
#define RX8_INTERP_U8_T_ZERO 0.0f

float rx8_interpolate_u8_table(int32_t index, const uint8_t *cells, float t)
{
    float v0;

    /* Read cell[index] first, always — matching the ROM, where the byte load
