...
| `suites` | `make -C web/ecu-emu test` (120 tests) + `make -C firmware check` + `make -C firmware/tests check` (tripwire gate + 22 harnesses) + `python3 web/explorer/build_site.py` and `make -C web/explorer check` | same four commands |
