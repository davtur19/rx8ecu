...
Generated by an autonomous analysis (script in `/tmp/opencode/rx8/map_final.py`,
not committed) over the 9 stock 512 KB ROMs (`roms/stock/*.bin`, SH-2E
big-endian) using `symbols/cal_tables.csv` (1210 entries) as the master list.
**No repository source file was modified; output only in `web/explorer/data/`.**
...
2. **content_match** — **exact** match of the table's 16-byte baseline window
   inside the target cal region (`[cal_lo, 0x7DAFF]`), with the candidate
   chosen consistently with the local drift curve (mode of the deltas of the
   unambiguous matches within ±0x800/±0x2000/±0x4000). Confidence high.
...
4. **hole** — tables that are 0xFF holes in the baseline (1 case: `Table 3D - 14_`
   @0x6EDF4): mapped only via drift, confidence low (exact matches are spurious).
...
Average confidence (excluding the baseline): high ≈ 700–950, medium ≈ 140–270,
...
- **32000 (N3M5E)** is the worst case: short cal span (0x71500→0x7DAFF),
  non-uniform drift, 17 unmapped tables and 775 low-confidence mappings. It
  should be redone with dedicated analysis (descriptors) if editorial use is
  required.
...
1. Extend `web/explorer/build_site.py` to read `table_addr_map_long.csv` and
   `roms_meta.json` and allow choosing the firmware model in the "Calibration
   Tables" tab (values extracted from the chosen ROM + mapped addresses).
