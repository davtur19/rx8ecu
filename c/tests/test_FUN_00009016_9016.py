#!/usr/bin/env python3
"""Differential test for FUN_00009016 (0x9016) — mem lift (RAM-only, straight-line), 34 bytes.
...
Run from repo root: python3 c/tests/test_FUN_00009016_9016.py
"""
...
ENTRY = 0x9016
RAW = bytes.fromhex("27 00 D2 3E 60 41 20 29 24 01 D1 3D 60 51 20 19 25 01 60 60 C9 8F CB 10 26 00 D3 3A 60 41 20 39 24 01")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7, ram, stack_top):
    ...
    r[0] = s16(_rdw(ram, r[5], 2))
    # 0x009024: op 0x2019
    r[0] = (r[0] & r[1]) & 0xFFFFFFFF
    # 0x009026: mov.w r0,@r5
    _wrw(ram, r[5], 2, r[0])
    # 0x009028: mov.b @r6,r0
    r[0] = s8(_rdw(ram, r[6], 1))
    # 0x00902A: op 0xC98F
    r[0] = (r[0] & 0x8F) & 0xFFFFFFFF
    # 0x00902C: op 0xCB10
    r[0] = (r[0] | 0x10) & 0xFFFFFFFF
