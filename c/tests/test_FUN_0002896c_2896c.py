#!/usr/bin/env python3
"""Differential test for FUN_0002896c (0x2896C) — mem lift (RAM-only, straight-line), 8 bytes.
...
Run from repo root: python3 c/tests/test_FUN_0002896c_2896c.py
"""
...
ENTRY = 0x2896C
RAW = bytes.fromhex("69 F6 6A F6 6B F6 6C F6")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7, ram, stack_top):
    ...
    r[9] = _rdw(ram, STACK_BASE + 0x400, 4)
    sp = (sp + 4) & 0xFFFFFFFF
    # 0x02896E: mov.l @r15+,r10
    r[10] = _rdw(ram, STACK_BASE + 0x404, 4)
    sp = (sp + 4) & 0xFFFFFFFF
    # 0x028970: mov.l @r15+,r11
    r[11] = _rdw(ram, STACK_BASE + 0x408, 4)
    sp = (sp + 4) & 0xFFFFFFFF
    # 0x028972: mov.l @r15+,r12
    r[12] = _rdw(ram, STACK_BASE + 0x40C, 4)
    sp = (sp + 4) & 0xFFFFFFFF
    r[15] = sp
