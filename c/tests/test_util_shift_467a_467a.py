#!/usr/bin/env python3
"""Differential test for util_shift_467a (0x467A) — pure straight-line lift, 14 bytes.
...
Run from repo root: python3 c/tests/test_util_shift_467a_467a.py
"""
...
ENTRY = 0x467A
RAW = bytes.fromhex("40 21 40 21 40 21 40 21 40 21 40 21 40 21")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7):
    ...
    # 0x00467A: op 0x4021
    T = r[0] & 1
    r[0] = ((r[0] >> 1) | (r[0] & 0x80000000)) & 0xFFFFFFFF
    # 0x00467C: op 0x4021
    T = r[0] & 1
    r[0] = ((r[0] >> 1) | (r[0] & 0x80000000)) & 0xFFFFFFFF
    # 0x00467E: op 0x4021
