#!/usr/bin/env python3
"""
Test alternating_sensor_sm_08 (0x5D3E8) via SH-2E emulator.

State machine reached from ssvControl (0x225C8) and similar control
functions.  Decoded behavior (see docs/functions/alternating_sensor_sm_08.md):

  r6 = 0x60214                 ; struct base
  ptr = RAM32[0x60220]         ; [r6+0xC] output target pointer
  mask = RAM8[0x6021C]         ; [r6+8]  sensor mask

  if RAM8[0xFFFFD355] == 0:                    ; first block
      masked = RAM8[0xFFFFD3A8] & mask
      if RAM16[0xFFFFD350] == 0xE926:
          if masked != 0:
              RAM8[@ptr] = RAM8[0xFFFFD354]
              if RAM8[0xFFFFD354] == 7:
                  RAM8[0xFFFFD387] = (RAM16[0xFFFFD352] >> 8) & 0xFF
              RAM8[0xFFFFD355] = 1
          else:
              RAM8[@ptr] = 0
              RAM8[0xFFFFD355] = 2
  ...
  out = RAM8[@ptr]
  if out == 0:
      RAM8[0xFFFFD387] = (r4 == 1) ? 1 : 0
      return r4
  if out == 5 or out == 7:
      return (RAM8[0xFFFFD387] == 1) ? 1 : 0
  return r4
"""
...
def ref(ram_in, r4):
    ...
    out = rb(ram, ptr)
    if out == 0:
        ram[0xFFFFD387] = 1 if (r4 & 0xFF) == 1 else 0
        ret = r4 & 0xFF
    elif out in (5, 7):
        ret = 1 if rb(ram, 0xFFFFD387) == 1 else 0
    else:
        ret = r4 & 0xFF
    return ret, ram
...
def main():
    ...
    for _ in range(20000):
        init = {}
        # struct area at 0x60214 (must override ROM defaults)
        init[0x6021C] = random.choice([0, 1, 0xFF, 0x0F, 0x80])
        ...
        init[0xFFFFD387] = random.randrange(0, 256)
        ...
        for cell in (0xFFFFD355, 0xFFFFD387, PTR_CELL):
            if rb(cpu.ram, cell) != rb(ref_ram, cell):
                bad.append('%s emu=%d ref=%d' % (hex(cell), rb(cpu.ram, cell), rb(ref_ram, cell)))
        if bad:
            fails += 1
            print("  0x5D3E8 FAIL r4=%d:" % r4, bad[:4], "in=", {hex(k): v for k, v in init.items() if k in (0xFFFFD355, 0xFFFFD350, 0xFFFFD352, 0xFFFFD354, 0xFFFFD3A8, 0xFFFFD387, 0x6021C, PTR_CELL)})
            if fails >= 5:
                break
    print(f"alternating_sensor_sm_08: {tests} tests, {fails} failures")
    print("ALT_SENSOR_SM:", "PASS" if fails == 0 else "FAIL")
