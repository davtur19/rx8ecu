#!/usr/bin/env python3
"""Differential test for diag_fault_cond_eval_903c (0x56788) — mem lift (RAM-only, straight-line), 8 bytes.
...
Run from repo root: python3 c/tests/test_diag_fault_cond_eval_903c_56788.py
"""
...
ENTRY = 0x56788
RAW = bytes.fromhex("2F 02 D3 15 60 30 60 0C")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7, ram, stack_top):
    ...
    local[0x400] = r[0]
    _wrw(ram, STACK_BASE + 0x400, 4, r[0])
    # 0x05678A: op 0xD315
    r[3] = 0xFFFFD210
    # 0x05678C: mov.b @r3,r0
    r[0] = s8(_rdw(ram, 0xFFFFD210, 1))
    # 0x05678E: op 0x600C
    r[0] = r[0] & 0xFF
    r[15] = sp
