#!/usr/bin/env python3
"""Differential test for FUN_00052854 (0x52854) — mem lift (RAM-only, straight-line), 8 bytes.
...
Run from repo root: python3 c/tests/test_FUN_00052854_52854.py
"""
...
ENTRY = 0x52854
RAW = bytes.fromhex("86 F4 86 F6 86 F8 86 FA")
SEED = 42
N = 2000
STACK_BASE = 0xFFFFD000
STACK_TOP = STACK_BASE + 0x400
STACK_OFFS = (0x410, 0x418, 0x420, 0x428)
RAM_MIN = None
RAM_MAX = None
...
def spec_mirror(r4, r5, r6, r7, ram, stack_top):
    ...
    r[0] = _rdw(ram, STACK_BASE + 0x410, 4)
    # 0x052856: mov.l @(0x18,r15),r0
    r[0] = _rdw(ram, STACK_BASE + 0x418, 4)
    # 0x052858: mov.l @(0x20,r15),r0
    r[0] = _rdw(ram, STACK_BASE + 0x420, 4)
    # 0x05285A: mov.l @(0x28,r15),r0
    r[0] = _rdw(ram, STACK_BASE + 0x428, 4)
    r[15] = sp
