#!/usr/bin/env python3
"""Differential test for obd_service_handler_6C166 (0x6C166) — pure straight-line lift, 10 bytes.
...
Run from repo root: python3 c/tests/test_obd_service_handler_6C166_6c166.py
"""
...
ENTRY = 0x6C166
RAW = bytes.fromhex("C8 08 00 07 C8 2C 00 07 C8 44")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7):
    ...
    # 0x06C166: op 0xC808
    T = 1 if (r[0] & 0x08) == 0 else 0
    # 0x06C168: op 0x0007
    macl = (s32(r[0]) * s32(r[0])) & 0xFFFFFFFF
    # 0x06C16A: op 0xC82C
    T = 1 if (r[0] & 0x2C) == 0 else 0
    # 0x06C16C: op 0x0007
    macl = (s32(r[0]) * s32(r[0])) & 0xFFFFFFFF
    # 0x06C16E: op 0xC844
    T = 1 if (r[0] & 0x44) == 0 else 0
