#!/usr/bin/env python3
"""
Verify shift_right_logical_r0 (0x44E0) against the ACTUAL ROM bytes, run in
the SH-2E emulator.  Value in r0, shift count in r1, result in r0.

C:
  uint32_t shift_right_logical_r0(uint32_t val, int32_t cnt)
  // cnt < 0 -> val ; cnt >= 32 -> 0 ; else val >> cnt (zero-fill)

Run from repo root:  python3 c/tests/test_shift_right_logical_r0.py [N]
"""
...
def ref(val, cnt):
    """Logical right shift with count clamping (zero-fill)."""
    if cnt < 0: return val
    if cnt >= 32: return 0
...
def main():
    ...
    edge_cases = [
        (0x00000000, 0), (0x00000001, 1), (0x80000000, 1),
        (0xFFFFFFFF, 1), (0xFFFFFFFF, 31), (0xFFFFFFFF, 0),
        (0x80000000, 8), (0x80000000, 16), (0x80000000, 24),
        (0x80000000, 31), (0xABCDEF01, 28), (0x12345678, 12),
    ...
    if err:
        ...
        sys.exit(1)
    print("OK  shift_right_logical_r0 @0x%04X  (%d edge + %d random inputs)" % (ENTRY, len(edge_cases), N))
    sys.exit(0)
