#!/usr/bin/env python3
"""Differential test for sensor_abs_deviation_44B9A (0x44B9A) — v3 lift (branches + delay slots), 8 bytes.
...
Run from repo root: python3 c/tests/test_sensor_abs_deviation_44B9A_44b9a.py
"""
...
ENTRY = 0x44B9A
RAW = bytes.fromhex("4F 26 4F 26 00 0B 00 09")
SEED = 0
N = 2000
MAXSTEPS = 100000
...
SEED_OFFS = (0x400, 0x404)   # lds.l pr/macl/mach slots seeded with the return sentinel
RAM_MIN = None
RAM_MAX = None
SPAN_END = 0x44BA2
SENT = 0x44BA2     # stop-sentinel, placed past the literal pool
...
CODE = {
    0x44b9a: {"kind": 'mem', "py": 'pr = _rdw(ram, STACK_BASE + 0x400, 4)\nsp = (sp + 4) & 0xFFFFFFFF', "slot_py": None, "target": None, "cond": None},
    0x44b9c: {"kind": 'mem', "py": 'pr = _rdw(ram, STACK_BASE + 0x404, 4)\nsp = (sp + 4) & 0xFFFFFFFF', "slot_py": None, "target": None, "cond": None},
