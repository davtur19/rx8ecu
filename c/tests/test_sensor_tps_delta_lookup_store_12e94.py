#!/usr/bin/env python3
"""Differential test for sensor_tps_delta_lookup_store (0x12E94) — pure straight-line lift, 10 bytes.
...
Run from repo root: python3 c/tests/test_sensor_tps_delta_lookup_store_12e94.py
"""
...
ENTRY = 0x12E94
RAW = bytes.fromhex("00 07 2B E8 00 07 2B EC 00 07")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7):
    ...
    # 0x012E96: op 0x2BE8
    T = 1 if (r[11] & r[14]) == 0 else 0
    # 0x012E98: op 0x0007
    macl = (s32(r[0]) * s32(r[0])) & 0xFFFFFFFF
    # 0x012E9A: op 0x2BEC
    x = r[14] ^ r[11]
    y = ((x - 0x01010101) & (~x) & 0xFFFFFFFF)
    T = 1 if (y & 0x80808080) else 0
    # 0x012E9C: op 0x0007
    macl = (s32(r[0]) * s32(r[0])) & 0xFFFFFFFF
