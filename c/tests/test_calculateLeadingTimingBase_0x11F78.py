#!/usr/bin/env python3
"""test_calculateLeadingTimingBase_0x11F78.py

Differential test for ROM 0x11F78 (60E0FC00.bin) — lift
c/calculateLeadingTimingBase_0x11F78.c.

Runs the ACTUAL ROM bytes of 0x11F78 — including the real sub-calls
...
Entry-point / range note: 0x11F78 IS the real entry point (function-pointer
slot @0x14404 of the engineControlCalculateTiming dispatcher 0x141FC table;
valid prologue; preceding function getKnownBooleanValue ends rts @0x11F6C).
The CSV range 0x11F78..0x1202A is CORRECT: code runs to rts @0x12026 (delay
@0x12028), the trailing twin 0x1202A starts exactly at the CSV end.

Key semantic facts (see the lift header): void leading-timing "base" writer:
  A604 = TwoDLookup(0x67908, A7AC)                6-pt RPM map
  A608 = ThreeDLookup(0x67958, C0D8, B594)         8x7 load x RPM
  A610 = TwoDLookup(0x6791C, A9FC)                 9-pt temp map
  A5F4 = 0.0 - A608*A610 + A604
  A5FC = (CC2C == 0) ? 80.0 : 16.0
  A60C = ThreeDLookup(0x67974, C0D8, B594)         20x18 load x RPM
  A614 = ThreeDLookup(0x679C8, B594, A9FC)         4x3 RPM x temp (f32 cells)
  A5EC = saturateLow( 0.0 + min(A60C, A5FC), A614 )
r0 at return = 4 * x-axis index of f32@B594 on desc 0x679C8 (the last helper
...
Run: python3 c/tests/test_calculateLeadingTimingBase_0x11F78.py [N]
...
# ---- RAM addresses (see c/calculateLeadingTimingBase_0x11F78.c header) ----
...
A604 = 0xFFFFA604; A608 = 0xFFFFA608; A610 = 0xFFFFA610
A5F4 = 0xFFFFA5F4; A5FC = 0xFFFFA5FC; A60C = 0xFFFFA60C
A614 = 0xFFFFA614; A5EC = 0xFFFFA5EC

FLOAT_IN  = [A7AC, B594, C0D8, A9FC]
FLOAT_OUT = [A604, A608, A610, A5F4, A5FC, A60C, A614, A5EC]

# last 3D lookup descriptor (for the r0 = ix*4 expectation)
DESC_LAST = 0x679C8   # 4x3 RPM x temp, type-0 f32 cells

# ROM f32 constants used as addends/clamps
def romf(rom, a):
    return struct.unpack('>f', rom[a:a + 4])[0]
R_6DA54 = romf  # 0.0   (A5F4 base addend)
R_6DA58 = romf  # 80.0  (A5FC clamp when CC2C == 0)
R_6DA5C = romf  # 16.0  (A5FC clamp when CC2C != 0)
R_6DA60 = romf  # 0.0   (A5EC addend)
...
def ref(cpu2, m, rom):
    """Line-for-line mirror of calculateLeadingTimingBase_0x11F78().
    ...
    # ---- lookup group 1 ----
    cpu2.call(0x2068, r4=0x67908, fr={4: a7ac})
    wrf(m, A604, cpu2.fr[0])
    cpu2.call(0x20DC, r4=0x67958, fr={4: c0d8, 5: rpm})
    wrf(m, A608, cpu2.fr[0])
    cpu2.call(0x2068, r4=0x6791C, fr={4: a9fc})
    wrf(m, A610, cpu2.fr[0])

    # ---- A5F4 = 0.0 - A608*A610 + A604 ----
    a5f4 = ts(ts(ts(R_6DA54(rom, 0x6DA54)) - ts(r32(m, A608) * r32(m, A610)))
              + r32(m, A604))
    wrf(m, A5F4, a5f4)

    # ---- A5FC = (CC2C == 0) ? 80.0 : 16.0 ----
    a5fc = R_6DA58(rom, 0x6DA58) if cc2c == 0 else R_6DA5C(rom, 0x6DA5C)
    wrf(m, A5FC, a5fc)

    # ---- lookup group 2 ----
    cpu2.call(0x20DC, r4=0x67974, fr={4: c0d8, 5: rpm})
    wrf(m, A60C, cpu2.fr[0])
    cpu2.call(0x20DC, r4=0x679C8, fr={4: rpm, 5: a9fc})
    wrf(m, A614, cpu2.fr[0])

    # ---- A5EC = saturateLow( 0.0 + min(A60C, A5FC), A614 ) ----
    cpu2.call(0x23F4, fr={4: r32(m, A60C), 5: a5fc})
    vmin = cpu2.fr[0]
    cpu2.call(0x23E4, fr={4: ts(R_6DA60(rom, 0x6DA60) + vmin), 5: r32(m, A614)})
    wrf(m, A5EC, cpu2.fr[0])

    # ---- r0 on return = 4 * ix(B594 on desc 0x679C8's X axis) ----
...
def main():
    ...
    assert struct.unpack('>f', rom[0x6DA54:0x6DA58])[0] == 0.0
    assert struct.unpack('>f', rom[0x6DA58:0x6DA5C])[0] == 80.0
    assert struct.unpack('>f', rom[0x6DA5C:0x6DA60])[0] == 16.0
    assert struct.unpack('>f', rom[0x6DA60:0x6DA64])[0] == 0.0

    cpu = SH2(rom)
    cpu2 = SH2(rom)          # dedicated instance for the helper leaves in ref()
    seeds = (0x11F78, 0x1202A, 0x679C8, 0xB594, 0x6DA5C)
    total_fails = 0
    ...
    if total_fails:
        ...
        sys.exit(1)
    print('OK  0x11F78 calculateLeadingTimingBase '
          '(%d random inputs across %d seeds)' % (N * len(seeds), len(seeds)))
    print('\nAll calculateLeadingTimingBase_0x11F78 tests passed.')
    sys.exit(0)
