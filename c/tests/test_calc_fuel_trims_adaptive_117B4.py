#!/usr/bin/env python3
"""
test_calc_fuel_trims_adaptive_117B4.py — differential bit-exact test of the
adaptive fuel-trim "outer" wrapper @0x117B4 (lift:
c/calc_fuel_trims_adaptive_117B4.c).

The outer is a pure task-dispatch skeleton: 10 jsr's in fixed ROM order then a
TAIL jmp into the 11th task (0x39302) whose delay-slot `lds.l @r15+,pr` returns
...
the (huge, not-yet-lifted) cumulative RAM effects of the 11 subsystem tasks.

Per the repo's dispatch/tail-call wrapper pattern (test_calc_lambda_feedback_pid_11A34.py,
harness_task_flag_run_c.py), the 11 callees are STUBBED on BOTH sides with
equivalent trace-append stubs so the OUTER's own behavior is pinned bit-exactly:
  ...
  * each stub appends its slot index k (0..10) to a trace buffer:
...
Compared bit-exactly, 0 mismatches required:
  * the whole 53-byte span 0xFFFFD12F..0xFFFFD163 (pins dispatch ORDER, CALL
    COUNT=11 and every store width; sign-extended wrap values 0xFE/0xFF land
...
r0 == 0x0 (deterministic with zeroed RAM) and that all 11 targets are entered
in ROM order — i.e. the stubs did not mask any dispatch change at the outer
level.

Usage:  python3 c/tests/test_calc_fuel_trims_adaptive_117B4.py [N]
...
LIFT = os.path.join(ROOT, 'c', 'calc_fuel_trims_adaptive_117B4.c')
ADDR = 0x117B4

# 11 dispatched callees in the ROM's literal-pool order @0x1197C..0x119A4
# (10 jsr + 1 tail jmp; addresses verified byte-level against 60E1D400.bin)
DISPATCH = (0x1AB70, 0x3A064, 0x1475A, 0x27622, 0x24596, 0x2B08E,
            0x1ABF4, 0x35C84, 0x5F030, 0x16E08, 0x39302)
...
BUILD_DIR = os.path.join('/tmp', 'rx8-fuel-trims-117B4')
...
def model(pre):
    """Python model of the outer's observable behavior with the 11 trace-append
...
#define DEF_STUB(name, k) \
    void name(void) { trims_stub(k); }
DEF_STUB(coil_charging_control_task, 0)
DEF_STUB(sensor_monitoring_dispatcher_3A064, 1)
DEF_STUB(calc_intake_pressure_target_1475A, 2)
DEF_STUB(fuel_control_task_dispatcher_27622, 3)
DEF_STUB(engine_task_dispatcher_24596, 4)
DEF_STUB(table_axis_increment_2B08E, 5)
DEF_STUB(transmission_control_priority_1ABF4, 6)
DEF_STUB(task_sensor_fault_dispatch_35C84, 7)
DEF_STUB(fault_condition_check_5F030, 8)
DEF_STUB(solenoid_drive_control_16E08, 9)
DEF_STUB(trampoline_vdi_39302, 10)
#undef DEF_STUB
...
int main(void) {
    ...
    while (fgets(line, sizeof line, stdin)) {
        ...
        calc_fuel_trims_adaptive_117B4();
        for (int i = 0; i < SPAN_LEN; i++)
...
def build_oracle(cc='cc'):
    ...
    src = os.path.join(BUILD_DIR, 'oracle_117B4.c')
    with open(src, 'w') as f:
        f.write(ORACLE_C.replace('"LIFT_PATH"', '"%s"' % LIFT))
    exe = os.path.join(BUILD_DIR, 'oracle_117B4')
...
def main():
    ...
    cpu = SH2(rom)
    seeds = (0x117B4, 0x1AB70, 0xCAFE, 0x1234, 0x5555)
    ...
    if total_fails == 0:
        ...
        if ret != 0x0:
            print('FULL-CHAIN FAIL: r0 = 0x%X, want 0x0' % ret)
            total_fails += 1
    ...
    if oracle is not None:
        chan += ' + host-C oracle'
    print('OK  calc_fuel_trims_adaptive_117B4 (%d inputs across %d seeds, '
          '0 mismatches; channel: %s)' % (total_vecs, len(seeds), chan))
    print('    full-chain sanity: r0 = 0x0, all 11 tasks dispatched in ROM order')
    sys.exit(0)
