#!/usr/bin/env python3
"""test_output_spark_0x8DE6.py

Differential test for ROM 0x8DE6 (60E1D400.bin) — lift
c/output_spark_0x8DE6.c.  Merged-xmap name: "outputSpark1" (per-rotor-A LEAD
spark output).

Runs the ACTUAL ROM bytes of 0x8DE6 in tools/sh2emu.py over seeded RAM states
...
0x91FE ("ignitonSomethingCalc", the 60E1D400 instance of the helper the
0x8DAE/60E0FC00 lift knows as 0x91C6: wraps the spark angle and arms the
compare timer, and its 0xAA74/0xDAB4 sub-chain) is NOT hand-transcribed: it is
...
This function is byte-for-byte identical to outputSpark1 @0x8DAE in
60E0FC00.bin (lift c/output_spark_0x8DAE.c); the ROM images differ only in
code base address and in the arming-helper address (0x91FE vs 0x91C6, with
retrigger helper 0xAA74 / descriptor table 0xDAB4 vs 0x9440 / 0xD81C).

Semantics (see c/output_spark_0x8DE6.c header):
  ...
  3. ignitonSomethingCalc(index)  @0x91FE   # wrap angle + arm the compare timer
  4. setSR(saved_sr)                          # restore SR

gen() restricts index to 0..7 (the per-rotor/per-coil channel index): larger
indices index past the 0xDAB4 descriptor table used by the 0xAA74 retrigger
...
ADDR = 0x8DE6
CALC = 0x91FE                 # un-lifted callee (ignitonSomethingCalc, 60E1D400)

# ---- RAM addresses (see c/output_spark_0x8DE6.c header) ----
...
A0C4 = 0xFFFFA0C4   # u32 per-channel pointer table (read by 0x91FE / 0xAA74)
...
def ref(cpu2, index, value, ram):
    """Line-for-line mirror of output_spark_0x8DE6().
    ...
    # ---- step 3: callee 0x91FE (emulator-in-the-model) ----
...
def main():
    ...
    seeds = (0x8DE6, 0x91FE, 0xA0D8, 0xDAB4, 0x8E20)
    total_fails = 0
    ...
    if total_fails:
        ...
        sys.exit(1)
    print('OK  0x8DE6 outputSpark1/output_spark (%d random inputs across %d seeds)'
          % (N * len(seeds), len(seeds)))
    print('\nAll output_spark_0x8DE6 tests passed.')
    sys.exit(0)
