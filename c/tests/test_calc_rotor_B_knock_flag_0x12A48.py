#!/usr/bin/env python3
"""test_calc_rotor_B_knock_flag_0x12A48.py

Differential test for ROM 0x12A48 (60E1D400.bin) — lift
c/calc_rotor_B_knock_flag_0x12A48.c.

Runs the ACTUAL ROM bytes of 0x12A48 in tools/sh2emu.py over seeded RAM states
...
Entry-point note: 0x12A48 IS the real entry point — the only ROM reference is
the function-pointer slot @0x14854 in the dispatcher engineControlCalculateTiming
(0x14584) dispatch table (c/engineControlCalculateTiming.c line 268).  Valid
prologue / rts+delay at 0x12B44/0x12B46; no branches into the body.

Key semantic facts (see the lift header):
  * void function — RAM side effects:
      u8@0xFFFFA66F  = (f32@C12C > 0.5f) ? 1 : ((f32@C12C > 0.45f) ? unchanged : 0)
      f32@0xFFFFA668 = knock-retard amount picked from the gates below
  * Three window-out calls on 0x2440 with (v=..., c=0.0, eps=1e-5@ROM 0x12AF4):
      w1 = window(f32@A764), w2 = window(f32@A674), w3 = window(f32@CA14)
  * f32@A668 selection:
      u8@AADA == 1                -> (w2==0) ? -25.0 : -20.0
      else if w3==0 and w1==0     -> -69.2
      else                        -> (u8@A66F==1) ? -69.2 : -20.0
    ROM constants: -25.0@0x6E418, -20.0@0x6E428, -69.2@0x6E41C,
                   -69.2@0x6E420, -20.0@0x6E42C.
...
Run: python3 c/tests/test_calc_rotor_B_knock_flag_0x12A48.py [N]
...
# ---- RAM addresses (see c/calc_rotor_B_knock_flag_0x12A48.c header) ----
C12C = 0xFFFFC12C   # f32 hysteresis input (fr15)
A764 = 0xFFFFA764   # f32 rotor-B knock delta 1 (w1 input)
A674 = 0xFFFFA674   # f32 rotor-B knock delta 2 (w2 input)
CA14 = 0xFFFFCA14   # f32 knock-control output (w3 input)
AADA = 0xFFFFAADA   # u8  rotor-B enable gate (==1)
A66F = 0xFFFFA66F   # u8  knock flag (read+write)
A668 = 0xFFFFA668   # f32 knock-retard output

ROM_EPS   = 0x00012AF4   # f32 1e-5 (window epsilon)
ROM_6E404 = 0x0006E404   # f32 0.5 (hysteresis high threshold)
ROM_6E408 = 0x0006E408   # f32 0.05 (hysteresis band width)
ROM_6E418 = 0x0006E418   # f32 -25.0
ROM_6E428 = 0x0006E428   # f32 -20.0
ROM_6E41C = 0x0006E41C   # f32 -69.2
ROM_6E420 = 0x0006E420   # f32 -69.2
ROM_6E42C = 0x0006E42C   # f32 -20.0
...
def ref(cpu2, m, rom, eps=ROM_EPS):
    """Line-for-line mirror of calc_rotor_B_knock_flag_0x12A48().
    ...
    fr15 = rdf(m, C12C)                  # fmov.s @r3,fr15 @0x12A60

    # 3x window-out 0x2440 (fr4=v, fr5=0.0, fr6=eps)
    cpu2.call(0x2440, fr={4: rdf(m, A764), 5: 0.0, 6: rdfb(rom, eps)}, ram=m)
    m = dict(cpu2.ram); w1 = cpu2.r[0]
    cpu2.call(0x2440, fr={4: rdf(m, A674), 5: 0.0, 6: rdfb(rom, eps)}, ram=m)
    m = dict(cpu2.ram); w2 = cpu2.r[0]
    cpu2.call(0x2440, fr={4: rdf(m, CA14), 5: 0.0, 6: rdfb(rom, eps)}, ram=m)
    m = dict(cpu2.ram); w3 = cpu2.r[0]

    # 0x12A88/0x12A9A: flag hysteresis — fcmp/gt fr4,fr15 -> T=(fr15>fr4)
    ...
    # 0x12AA4..0x12B38: knock-retard amount into f32@A668
    if m.get(AADA, 0) == 1:
        wrf(m, A668, rdfb(rom, ROM_6E418) if w2 == 0 else rdfb(rom, ROM_6E428))
    elif w3 == 0 and w1 == 0:
        wrf(m, A668, rdfb(rom, ROM_6E41C))
    else:
        wrf(m, A668, rdfb(rom, ROM_6E420) if m.get(A66F, 0) == 1
            else rdfb(rom, ROM_6E42C))
    return m
...
def gen_state(rng):
    ...
    AADA covers 1/0/other; A66F gets 0,1,junk and the output float is junk so a
    ...
    ram[A66F] = rng.choice([0, 1, 0x55, 0xFF, rng.randint(0, 255)])

    # output float: junk so a missed write is caught
    setf(ram, A668, rng.uniform(-1000.0, 1000.0))
    return ram
...
def main():
    ...
    seeds = (0x12A48, 0x128C4, 0x128FE, 0xA66F, 0x6E41C)
    total_fails = 0
            ...
            if bad:
                print('MISMATCH seed=0x%X iter=%d: %s' %
                      (seed, it, {hex(k): (hex(g), hex(e))
                                  for k, g, e in bad[:12]}))
                print('  C12C=%r AADA=%d A66F=%r w1=%r w2=%r w3=%r' % (
                    rdf(ram, C12C), ram.get(AADA, 0), ram.get(A66F, 0),
                    None, None, None))
                fails += 1
                if fails >= 3:
                    break
    ...
    if total_fails:
        ...
        sys.exit(1)
    print('OK  0x12A48 calc_rotor_B_knock_flag '
          '(%d random inputs across %d seeds)' % (N * len(seeds), len(seeds)))
    print('\nAll calc_rotor_B_knock_flag_0x12A48 tests passed.')
    sys.exit(0)
