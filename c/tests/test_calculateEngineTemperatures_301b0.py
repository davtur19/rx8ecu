#!/usr/bin/env python3
"""Differential test for calculateEngineTemperatures (0x301B0) — v3 lift + v4 FPU, 70 bytes.
...
Run from repo root: python3 c/tests/test_calculateEngineTemperatures_301b0.py
"""
...
ENTRY = 0x301B0
RAW = bytes.fromhex("93 3F F6 9D D1 23 F3 6C F4 38 92 3B D4 20 F5 28 F0 48 F3 01 D4 20 F2 0C D3 20 F2 52 F0 4C F2 3E F1 2A F3 6C F0 48 F3 01 D4 1D F1 0C F1 52 F0 4C F1 3E F3 1A F0 48 F6 01 F3 0C F3 52 D2 19 F0 4C F3 6E 00 0B F2 3A")
SEED = 42
N = 500
MAXSTEPS = 100000
...
RAM_MAX = 0xFFFFBE18
SPAN_END = 0x301F6
SENT = 0x301F6     # stop-sentinel, placed past the literal pool
