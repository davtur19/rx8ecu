#!/usr/bin/env python3
"""Differential test for build_be32_from_bytes (0xF4) — mem lift (RAM-only, straight-line), 12 bytes.
...
Run from repo root: python3 c/tests/test_build_be32_from_bytes_f4.py
"""
...
ENTRY = 0xF4
RAW = bytes.fromhex("84 41 60 0C 62 40 40 28 63 03 62 2C")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7, ram, stack_top):
    ...
    r[0] = s8(_rdw(ram, (r[4] + 1), 1))
    # 0x0000F6: op 0x600C
    r[0] = r[0] & 0xFF
    # 0x0000F8: mov.b @r4,r2
    r[2] = s8(_rdw(ram, r[4], 1))
    # 0x0000FA: op 0x4028
    r[0] = (r[0] << 16) & 0xFFFFFFFF
    # 0x0000FC: op 0x6303
    r[3] = r[0]
    # 0x0000FE: op 0x622C
    r[2] = r[2] & 0xFF
