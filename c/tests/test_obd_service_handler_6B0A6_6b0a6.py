#!/usr/bin/env python3
"""Differential test for obd_service_handler_6B0A6 (0x6B0A6) — pure straight-line lift, 10 bytes.
...
Run from repo root: python3 c/tests/test_obd_service_handler_6B0A6_6b0a6.py
"""
...
ENTRY = 0x6B0A6
RAW = bytes.fromhex("77 00 00 07 77 24 00 07 77 38")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7):
    ...
    # 0x06B0A6: op 0x7700
    r[7] = (r[7] + s8(0x00)) & 0xFFFFFFFF
    # 0x06B0A8: op 0x0007
    macl = (s32(r[0]) * s32(r[0])) & 0xFFFFFFFF
    # 0x06B0AA: op 0x7724
    r[7] = (r[7] + s8(0x24)) & 0xFFFFFFFF
    # 0x06B0AC: op 0x0007
    macl = (s32(r[0]) * s32(r[0])) & 0xFFFFFFFF
    # 0x06B0AE: op 0x7738
    r[7] = (r[7] + s8(0x38)) & 0xFFFFFFFF
