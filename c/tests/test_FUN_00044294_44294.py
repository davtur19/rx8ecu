#!/usr/bin/env python3
"""Differential test for FUN_00044294 (0x44294) — pure straight-line lift, 18 bytes.
...
Run from repo root: python3 c/tests/test_FUN_00044294_44294.py
"""
...
ENTRY = 0x44294
RAW = bytes.fromhex("CA 94 CA 80 CA A8 CA AF CA 44 CA 5C CA 48 CA B9 CA B8")
SEED = 42
N = 2000
...
def spec_mirror(r4, r5, r6, r7):
    ...
    # 0x044294: op 0xCA94
    r[0] = (r[0] ^ 0x94) & 0xFFFFFFFF
    # 0x044296: op 0xCA80
    r[0] = (r[0] ^ 0x80) & 0xFFFFFFFF
    # 0x044298: op 0xCAA8
    r[0] = (r[0] ^ 0xA8) & 0xFFFFFFFF
    # 0x04429A: op 0xCAAF
    r[0] = (r[0] ^ 0xAF) & 0xFFFFFFFF
    # 0x04429C: op 0xCA44
    r[0] = (r[0] ^ 0x44) & 0xFFFFFFFF
    # 0x04429E: op 0xCA5C
    r[0] = (r[0] ^ 0x5C) & 0xFFFFFFFF
    # 0x0442A0: op 0xCA48
    r[0] = (r[0] ^ 0x48) & 0xFFFFFFFF
    # 0x0442A2: op 0xCAB9
    r[0] = (r[0] ^ 0xB9) & 0xFFFFFFFF
    # 0x0442A4: op 0xCAB8
    r[0] = (r[0] ^ 0xB8) & 0xFFFFFFFF
