/* ROM: 60E0FC00 | Address: 0x1A534 | Size: 344 bytes | STATUS: DRAFT
 * Auto-generated by tools/gen_c_lift_v8.py — ST callee (CFG).
 * Never replaces c/*.c. */
#include <stdint.h>
typedef struct {
    uint32_t r[16];
    uint32_t pr, T, Q, M, macl, mach, sr, vbr, gbr, fpul, fpscr;
    uint32_t fr[16];   /* FPU bit patterns (IEEE-754) */
    uint32_t ram_base; /* bank base (0 for 60E1D400-style flat test) */
} ST;

void f_3920(ST *s);
void f_3934(ST *s);
void f_1ACAE(ST *s);
void f_1AD8C(ST *s);
void f_1A534(ST *s)
{
    uint32_t local_3f8 = 0;
    uint32_t local_3fc = 0;
    /* 0x01A534: sts.l pr,@-r15 */
    local_3fc = s->pr;
    /* 0x01A536: op 0x7FFC */
    s->r[15] = s->r[15] + (uint32_t)(int32_t)(int8_t)0xFC;
    /* 0x01A538: op 0xD343 */
    s->r[3] = 0x00003920u;
    /* 0x01A53A: jsr @r3 */
    /* 0x01A53C: op 0xE410 */
    s->pr = 0x0001A53E;
    s->r[4] = (uint32_t)(int32_t)(int8_t)0x10;
    f_3920(s);
    /* 0x01A53E: op 0xD351 */
    s->r[3] = 0x0001ACAEu;
    /* 0x01A540: jsr @r3 */
    /* 0x01A542: mov.l r0,@r15 */
    s->pr = 0x0001A544;
    local_3f8 = s->r[0];
    f_1ACAE(s);
    /* 0x01A544: op 0xD250 */
    s->r[2] = 0x0001AD8Cu;
    /* 0x01A546: jsr @r2 */
    /* 0x01A548: op 0x0009 */
    s->pr = 0x0001A54A;
    
    f_1AD8C(s);
    /* 0x01A54A: mov.l @r15+,r4 */
    s->r[4] = local_3f8;
    /* 0x01A54C: op 0xD342 */
    s->r[3] = 0x00003934u;
    /* 0x01A54E: jmp @r3 (tail) */
    /* 0x01A550: lds.l @r15+,pr */
    s->pr = local_3fc;
    f_3934(s);
    return;
    return; /* fallthrough */
}
