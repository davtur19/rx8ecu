/* ROM: 60E0FC00 | Address: 0x661E | Size: 42 bytes | STATUS: DRAFT
 * Auto-generated by tools/gen_c_lift_v7.py — ST (shared state) ABI,
 * leaf callee library (c/lib/f_<hex>.c). Never replaces c/*.c. */
#include <stdint.h>
typedef struct {
    uint32_t r[16];
    uint32_t pr, T, Q, M, macl, mach, sr, gbr, fpul, fpscr;
    uint32_t fr[16];   /* FPU bit patterns (IEEE-754) */
    uint32_t ram_base; /* bank base (0 for 60E1D400-style flat test) */
} ST;

void f_661E(ST *s)
{
    uint32_t local_3f8 = 0;
    uint32_t local_3fc = 0;
    /* 0x00661E: sts.l pr,@-r15 */
    local_3fc = s->pr;
    /* 0x006620: op 0xD331 */
    s->r[3] = 0x0000A266u;
    /* 0x006622: op 0x7FFC */
    s->r[15] = s->r[15] + (uint32_t)(int32_t)(int8_t)0xFC;
    /* 0x006624: jsr @r3 */
    /* 0x006626: op 0xE401 */
    s->r[4] = (uint32_t)(int32_t)(int8_t)0x01;
    /* 0x006628: op 0xD334 */
    s->r[3] = 0x0000C3CEu;
    /* 0x00662A: jsr @r3 */
    /* 0x00662C: mov.b r0,@r15 */
    local_3f8 = s->r[0];
    /* 0x00662E: op 0xD334 */
    s->r[3] = 0x00011C1Cu;
    /* 0x006630: jsr @r3 */
    /* 0x006632: mov.b @r15,r4 */
    s->r[4] = (uint32_t)(int32_t)(int8_t)(local_3f8 & 0xFFu);
    /* 0x006634: op 0xD233 */
    s->r[2] = 0x00008320u;
    /* 0x006636: jsr @r2 */
    /* 0x006638: op 0x0009 */
    
    /* 0x00663A: op 0xD333 */
    s->r[3] = 0x00009110u;
    /* 0x00663C: jsr @r3 */
    /* 0x00663E: op 0x0009 */
    
    /* 0x006640: op 0xD21E */
    s->r[2] = 0x00003D58u;
    /* 0x006642: op 0x7F04 */
    s->r[15] = s->r[15] + (uint32_t)(int32_t)(int8_t)0x04;
    /* 0x006644: jmp @r2 (tail) */
    /* 0x006646: lds.l @r15+,pr */
    s->pr = local_3fc;
    return; /* fallthrough */
}
