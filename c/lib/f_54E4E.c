/* ROM: 60E0FC00 | Address: 0x54E4E | Size: 22 bytes | STATUS: DRAFT
 * Auto-generated by tools/gen_c_lift_v7.py — ST (shared state) ABI,
 * leaf callee library (c/lib/f_<hex>.c). Never replaces c/*.c. */
#include <stdint.h>
typedef struct {
    uint32_t r[16];
    uint32_t pr, T, Q, M, macl, mach, sr, gbr, fpul, fpscr;
    uint32_t fr[16];   /* FPU bit patterns (IEEE-754) */
    uint32_t ram_base; /* bank base (0 for 60E1D400-style flat test) */
} ST;

void f_54E4E(ST *s)
{
    uint32_t local_3f8 = 0;
    uint32_t local_3fc = 0;
    /* 0x054E4E: sts.l pr,@-r15 */
    local_3fc = s->pr;
    /* 0x054E50: op 0xD34F */
    s->r[3] = 0x00053804u;
    /* 0x054E52: op 0x7FFC */
    s->r[15] = s->r[15] + (uint32_t)(int32_t)(int8_t)0xFC;
    /* 0x054E54: jsr @r3 */
    /* 0x054E56: mov.b r4,@r15 */
    local_3f8 = s->r[4];
    /* 0x054E58: mov.b @r15,r4 */
    s->r[4] = (uint32_t)(int32_t)(int8_t)(local_3f8 & 0xFFu);
    /* 0x054E5A: op 0x6503 */
    s->r[5] = s->r[0];
    /* 0x054E5C: op 0xD338 */
    s->r[3] = 0x00055EEEu;
    /* 0x054E5E: op 0x7F04 */
    s->r[15] = s->r[15] + (uint32_t)(int32_t)(int8_t)0x04;
    /* 0x054E60: jmp @r3 (tail) */
    /* 0x054E62: lds.l @r15+,pr */
    s->pr = local_3fc;
    return; /* fallthrough */
}
