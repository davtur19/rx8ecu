 ...
 *   0x263DC  r10 = 0x2054 (saveSRMaskParam)   r11 = 0x2064 (restoreSR)
 ...
 *                      restoreSR(s1); saveSRMaskParam(&s2,0xE0);
 *                      reg16SetClear(0xF754,0x20,1); restoreSR(s2);
 *   OFF (else):        saveSRMaskParam(&s3,0xE0); reg16SetClear(0xF754,0x20,0);
 *                      restoreSR(s3); saveSRMaskParam(&s4,0xE0);
 *                      reg16SetClear(0xF754,0x40,0); restoreSR(s4);
...
void setImmoLight(uint8_t on)
{
    ...
    restoreSR(slot);   /* 0x26436: the trailing 0x2064 */
}
