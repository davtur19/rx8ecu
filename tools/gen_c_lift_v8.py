...
def _fmt_reason(reason):
    """Flatten an emit failure reason (str or nested tuple) into one line."""
    if isinstance(reason, tuple):
...
def run_batch(rom, outdir, catalog, bounds, n=20, seed=42, cases=500,
              rom_label=None, jobs=2, timeout=240, stub_on_fail=False):
    ...
    for e in entries[:n]:
        ...
        if fail:
            if stub_on_fail:
                _write_stub_lib(addr, fail, rom_label=rom_label)
                print('    STUB f_%X: %s' % (addr, _fmt_reason(fail)))
                summary.append((addr, 'STUB', _fmt_reason(fail)))
            else:
        ...
        if not ok:
            if stub_on_fail:
                _write_stub_lib(addr, reason, rom_label=rom_label)
                print('    STUB f_%X: %s' % (addr, _fmt_reason(reason)))
                summary.append((addr, 'STUB', _fmt_reason(reason)))
                continue
...
def main():
    ...
    if args.emit:
        ...
        if not ok:
            if args.stub_on_fail:
                ...
                print('STUB f_%X: %s -> %s' % (
                    addr, _fmt_reason(reason), stub_p))
                return 0
            print('EMIT FAILED: %s' % (reason,))
            return 1
    ...
    if args.n:
        ...
        summary = run_batch(rom, outdir, catalog, bounds, n=args.n,
                            ...
                            stub_on_fail=args.stub_on_fail)
        kept = [s for s in summary if s[1] == 'PASS']
        stubbed = [s for s in summary if s[1] == 'STUB']
        print('== batch summary: generated=%d passed=%d dropped=%d stubbed=%d' %
              (len(summary), len(kept),
               len(summary) - len(kept) - len(stubbed), len(stubbed)))
