#!/usr/bin/env python3
"""make_elf_fc00.py - build the big-endian SH-2E analysis ELF for the 60E0FC00 bank.

WHAT
----
Wraps the stock ROM ``roms/stock/60E0FC00.bin`` (512 KiB, raw bytes, big-endian)
...
Same proven layout as the 60E1D400 baseline (``tmp/ida/make_elf.py`` scratch).
...
USAGE
-----
  python3 tools/make_elf_fc00.py
  python3 tools/make_elf_fc00.py --out /tmp/60E0FC00_be.elf
  python3 tools/make_elf_fc00.py --rom roms/stock/60E0FC00.bin --dry-run

INPUT
-----
  --rom : stock ROM image (default: <repo>/roms/stock/60E0FC00.bin)

OUTPUT
------
  --out : ELF image written (default: <repo>/tmp/ida/60E0FC00_be.elf);
...
DEFAULT_ROM = REPO_ROOT / "roms" / "stock" / "60E0FC00.bin"
DEFAULT_OUT = REPO_ROOT / "tmp" / "ida" / "60E0FC00_be.elf"
...
def main(argv=None):
    ap = argparse.ArgumentParser(
        description="Build the big-endian SH-2E analysis ELF for the 60E0FC00 ROM bank."
    )
