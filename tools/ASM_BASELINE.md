...
This is the concrete first step of `../PLANS.md` Track B. It de-risks the whole
effort: the byte-exact oracle does not require you to find SHC. The reassembly of the
...
Method: SH-2 instructions are all 2 bytes. Every even offset in the code window
`0x800..0x60000` is decoded independently. A word becomes an instruction if it
re-encodes to the same 2 bytes, else a raw `.word`. Everything outside the window
(vectors, strings, Hitachi-OS data, calibration) is `.word` data. Branch/PC-relative
operands become `L_xxxxxx` labels; the whole ROM is one unit linked at VMA 0, so
ranges are original. A self-correcting loop forces any as-rejected or mis-encoding
word back to raw. The loop converges to `cmp == 0`.

The 5–10 raw fallbacks are DATA words capstone over-decodes as extended-SuperH ops
(`ldc.l @rn+,tbr`, `stc.l tbr,@-rn`, `synco`). sh-elf-as rejects them. This is extra
confirmation that the real code is plain SH-2 (SH-2E core). The words are emitted
verbatim, so byte-exactness holds.
...
round-trip — useful when you lift one function at a time:
...
- **Isolated slices** whose branches/pools point *outside* the slice cannot assemble:
  SH short branches (8/12-bit) cannot target undefined externals. The whole-ROM path
  (`tools/rom_rebuild.py`) avoids this. It assembles everything as one unit.
- **Code/data classification** recovers only *literal pools* (PC-relative
  refs). Jump tables and computed-address data come from the IDA/Ghidra maps.
  The `symbols/` tables and `analysis/data_regions_60E1D400.csv` encode that data.

## Next steps

All three original next steps (IDA/Ghidra symbol-map ingest, cross-ROM diff,
