...
# float_compare.same_result_bits — both-NaN compares sign only), fpul
# (integer conversions only — exact), pr and every written RAM
# address (T is not part of the state call() exposes — not compared).
# One extra raw-bits sNaN edge case (EDGE_NAN_BITS over FR0-15, caso=N)
# covers NaN quieting; Inf/denormals remain out of scope.
...
def emit_fpu_test(addr, name, size, rom, records, info, seed, out_t,
    ...
    test = (
        ...
        '(same_result_bits). Inf/denormals remain out of scope.\n'
        ...
        'Run from repo root: python3 c/tests/test_%s_%x.py\n'
        '"""\n'
        'import os, random, struct, sys, math\n\n'
        'ROOT = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))\n'
        'sys.path.insert(0, os.path.join(ROOT, "tools"))\n'
